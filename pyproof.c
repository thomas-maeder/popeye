...
void ProofInitialise(void) {
    ...
    BlockedBishopc1= ProofBoard[202] == fb
		  && ProofBoard[225] == pb
		  && ProofBoard[227] == pb;
    ...
    CapturedBishopc1= ProofBoard[202] != fb
		   && ProofBoard[225] == pb
		   && ProofBoard[227] == pb;

    CapturedBishopf1= ProofBoard[205] != fb
		   && ProofBoard[228] == pb
		   && ProofBoard[230] == pb;

    CapturedBishopc8= ProofBoard[370] != fn
		   && ProofBoard[345] == pn
		   && ProofBoard[347] == pn;

    CapturedBishopf8= ProofBoard[373] != fn
		   && ProofBoard[348] == pn
		   && ProofBoard[350] == pn;
...
smallint ProofBlKingMovesNeeded(void) {
	...
	if (castling_flag[nbply]&0x02) {		/* V3.37  NG */
	    ...
	    /* BlKingMoves is the number of moves the bl king
	       ...
	       but we might save a rook moves
	     */
	...
	if (p2 != vide) {
		...
		switch (p2) {
		  ...
		  case pb:
		    ...
		    break;
		  default:
		    if (  !(CapturedBishopc1 && *bnp == 202 && p2 == fb)
		...
		switch (p2) {
		  ...
		  case pn:
		    ...
		    break;
		  default:
		    if (  !(CapturedBishopc1 && *bnp == 202 && p2 == fn)
