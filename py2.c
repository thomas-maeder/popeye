...
/* id - case dep., ia - case arr., ip - case pri. */
{
	...
	if (CondFlag[newkoeko])                                 /* V3.1  TLi */
		if (nocontact(id, ia, ip) ^ nocontact(initsquare, id, initsquare))
			return false;
	return(true);
} /* end of legalsquare */
...
boolean hopimok(square i, square j, square k, smallint diff)    /* V3.12  TM */
/* hop i->j hopping over k in steps of diff ok? */
{
	...
	e[i]= vide;  /* an imitator might be disturbed by the moving hopper! */ 
...
void joueim(smallint diff)                      /* V2.4d  TM */
{
	...
	for (i= inum[nbply]; i > 0; isquare[--i]+= diff); /* V2.90  NG */
...
boolean rmhopech(square i, numvec kend, numvec kanf, smallint m, piece p, boolean (* evaluate)(square,square,square))	/* V2.1c, V3.62  NG */
{
	square j, j1;
	numvec k, k1;
	piece p1;
	 ...
	 *      kend == 8, kanf == 5: orthogonal types (rookmoose, rooksparrow, bishopeagle)
	 *      kend == 4, kanf == 1: diagonal types   (bishopmoose, bishopsparrow, rookeagle)
	 *
	 *      YES, this is definitely different to generating their moves ...  NG
	 */
...
boolean rcsech(square i, numvec k, numvec k1, piece p, boolean (* evaluate)(square,square,square))       /* V2.1c  NG */
{
	square j= i;
...
boolean cscheck(square i, piece p, boolean (* evaluate)(square,square,square))       /* V2.1c  NG */
{
	numvec  k;
...
boolean rrefcech(square i, square i1, smallint x, piece p, boolean (* evaluate)(square,square,square))   /* V2.1c  NG */
{
	square j;
	numvec k;
	...
	for (k= 3; k >= 0; k--)
		for (k1= 72; k1 >= 0; k1-= 24) {        /* V2.90  NG */
			if (e[j2 = j1 + k + k1] != vide)        /* V1.5c, V2.90  NG */
				if (e[j= (j2<<1) - i] == p)     /* V1.5c, V2.90  NG */
					if (i != j 		/* V3.54  NG */
...
boolean leap16check(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.38  NG */
{
	return leapcheck(i, 89, 96, p, evaluate);
}

boolean leap24check(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.42  TLi */
{
	return leapcheck(i, 97, 104, p, evaluate);
}

boolean leap35check(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.42  TLi */
{
	return leapcheck(i, 105, 112, p, evaluate);
}

boolean leap37check(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.34  TLi */
{
	return leapcheck(i, 81, 88, p, evaluate);
}

boolean okapicheck(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.32  TLi */
{
...
boolean grasshop2check(square i, piece p, boolean (* evaluate)(square,square,square))   /* V3.34  TLi */
{
	return rhop2check(i, 1, 8, p, evaluate);
}

boolean grasshop3check(square i, piece p, boolean (* evaluate)(square,square,square))   /* V3.34  TLi */
{
...
boolean kinghopcheck(square i, piece p, boolean (* evaluate)(square,square,square))
{
	return shopcheck(i, 1, 8, p, evaluate);       /* SE */
}

boolean doublegrascheck(square i, piece p, boolean (* evaluate)(square,square,square))  /* V3.44  SE */
{
...
boolean contragrascheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
...
boolean nightlocustcheck(square i, piece p,     	/* V3.53  TLi */
...
boolean tritoncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return marincheck(i, 1, 4, p, evaluate);
}

boolean nereidecheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return marincheck(i, 5, 8, p, evaluate);
}

boolean nightriderlioncheck(square i, piece p,
...
boolean t_lioncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return lrhopcheck(i, 1, 4, p, evaluate);
}

boolean f_lioncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
...
boolean rlh_check(square i, piece p, int type, numvec k, numvec k1,
		  boolean (* evaluate)(square,square,square))
/* detects check by rose hopper or lion on a single rose line defined by k
** and direction k1  -  V3.23  TLi
** Bug fixed -- TLi, V3.39
*/
{
square j = i;
...
boolean roselioncheck(square i, piece p, boolean (* evaluate)(square,square,square))
...
boolean rosehoppercheck(square i, piece p, boolean (* evaluate)(square,square,square))
	...
	if (p <= roin) {                                /* V3.47  NG */
		if ((i <= haut - 48) ||                 /* V3.02  TLi */
			...
			if (ep[nbply] && RB_[nbply] != rb) {            /* V3.45  TLi */
			    ...
			    if (e[sq+24] == pbn && (*evaluate)(sq+24, sq, rn)) imech(sq+24, sq);
			}
		}
		...
		if ((i >= bas + 48) ||                  /* V3.02  TLi */
			...
			if (ep[nbply] && RN_[nbply] != rn) {            /* V3.45  TLi */
			    /* ep captures of royal pawns */
			    square sq= ep[nbply];      /* square passed */
			    if (e[sq-24] == pbb && (*evaluate)(sq-24, sq, rn)) imech(sq-24, sq);
			}
		}
	}
	return false;
...
boolean bspawncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	...
	if ((p == bspawnn) || (CondFlag[blrefl_king] && p == roin)) {             /* V3.04  NG */
	...
	} else {        /* hopefully ((p == bspawnb) || (CondFlag[whrefl_king] && p == roib)) */  /* V3.04  NG */
...
boolean spawncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	...
	if ((p == spawnn) || (CondFlag[blrefl_king] && p == roin)) {              /* V3.04  NG */
		if (i <= haut - 24) {   /* it can move from eigth rank */
			finligne(i, 23, p1, j);
			if (p1 == p)
				if ((*evaluate)(j, i, i))       /* V3.02  TLi */
...
boolean dragoncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	if (leapcheck(i, 9, 16, p, evaluate))       /* V2.60  NG */
		return true;
	if ((p == dragonn) || (CondFlag[blrefl_king] && p == roin)) {             /* V3.04  NG */
	...
	} else {        /* hopefully ((p == dragonb) || (CondFlag[whrefl_king] && p == roib)) */  /* V3.04  NG */
...
boolean leap36check(square i, piece p, boolean (* evaluate)(square,square,square
)) /* V3.64  TLi */
{
...
boolean roicheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return leapcheck(i, 1, 8, p, evaluate);
}

boolean cavcheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return leapcheck(i, 9, 16, p, evaluate);
}

boolean damecheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return ridcheck(i, 1, 8, p, evaluate);
}

boolean tourcheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return ridcheck(i, 1, 4, p, evaluate);
}

boolean foucheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
	return ridcheck(i, 5, 8, p, evaluate);
}

boolean pioncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.60  NG */
{
...
boolean ep_not_libre(piece p, square sq, boolean generating,   /* V3.44  TLi */
		     boolean (*checkfunc)(square,piece,boolean (*)(square,square,square)))
/* Returns true if a pawn who has just crossed the square sq is paralysed
** by a piece p due to the ugly Madrasi-ep-rule by a pawn p. Dear inventors
** of fairys, make it as sophisticated and inconsistent as possible!
...
ply ply_dblstp= generating ? nbply-1 : nbply;
  ...
  return (((ep[ply_dblstp] == sq) || (ep2[ply_dblstp] == sq)) &&
	  nbpiece[p] &&
	  (*checkfunc)(sq, p, flaglegalsquare ? legalsquare : eval_ortho));
} /* end eplibre */
...
boolean libre(square sq, boolean generating) {
...
#ifdef NODEF
	if (TSTFLAG(PieSpExFlags, Neutral))   /* V3.50 SE N.B. Not just if p is neutral */
		if (p >= roib )		/* V3.51  NG */
			initneutre(noir);
		else
			initneutre(blanc);
	...
	switch (p) {                     /* V3.22  TLi */
	  ...
	  case pbn: /* black berolina pawn */
		...
		ep_not_libre(pbb, sq + 25, generating, pbcheck))
			flag= False;
	  /* NB: Super (Berolina) pawns cannot neither be captured ep
	  ** nor capture ep themselves.
	  */
	    break;
	}

	flag= flag && (!nbpiece[-p] || !(*checkfunctions[abs(p)])(sq, -p,
   ...
   if (CondFlag[eiffel]) {               /* V3.60  TLi */
	...
	switch (p) {                     /* V3.22  TLi */
        ...
        if (test)
	    flag= flag && (!nbpiece[ep] || !(*checkfunctions[abs(ep)])(sq, ep,
			    flaglegalsquare ? legalsquare : eval_ortho));
    } /* CondFlag[eiffel] */

#ifdef NODEF    /* V3.60  TLi */
...
boolean Result;
	...
	/* if (CondFlag[madras]) V3.32  TLi */
	if (flag_madrasi) {                    /* V3.32, V3.60  TLi */
...
numvec  k;
	...
	for (k= 8; k > 0; k--) {                /* V3.00  NG */
		...
		/* if (p1 == p && boolnoedge[j1] != boolnoedge[i] && */
		if (p1 == p &&     /* V3.22  TLi */
		    NoEdge(j1) != NoEdge(i) &&
...
boolean maooaridercheck(square i, piece p, numvec fir, numvec sec, boolean (* evaluate)(square,square,square))
{
square  j1, j2;
...
boolean maooariderlioncheck(square i, piece p, numvec fir, numvec sec,
               boolean (* evaluate)(square,square,square)) { /* V3.64  TLi */
        square  j1= i+fir, j2= i+sec;
...
boolean maoriderlioncheck(square i, piece p, boolean (* evaluate)(square,square,
square)) {
...
boolean moariderlioncheck(square i, piece p,
...
{   /* could be used for other genres e.g. Ohneschach */
	...
	for (j=8; j>0; j--, z--) {
		if (e[z]!=vide) {
			if (!libre(z, false)) return false;
		}
	}
    }

/* To avoid messing up the ???[nbply] arrays during output of the solution */
...
boolean eval_isardam(square id, square ia, square ip)       /* V3.44  SE/TLi */
{
	boolean flag=false;
	couleur camp;
	...
	** move from id, ia and ip.  TLi */
	...
	while (encore() && !flag) {
		...
		/* For neutral Ks will need to return true always */
			flag=jouecoup() && (camp==blanc ? !echecc(blanc) : !echecc(noir));
		else if (CondFlag[isardam])
			flag= jouecoup();
		/* Isardam + Brunner may be possible! in which case this logic is correct */
		repcoup();
	}
	finply();
	return flag;
...
boolean orixcheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V3.44  NG */
{
numvec  k;
piece   p1;
square  j, j1;
...
boolean leap15check(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.46  NG */
{
	return leapcheck(i, 113, 120, p, evaluate);
}

boolean leap25check(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.46  NG */
{
	return leapcheck(i, 121, 128, p, evaluate);
}

boolean gralcheck(square i, piece p, boolean (* evaluate)(square,square,square)) /* V3.46  NG */
{
...
/* id - case dep., ia - case arr., ip - case pri. */
    ...
    return flaglegalsquare ? legalsquare(id, ia, ip) : eval_ortho(id, ia, ip);
} /* aux_whx */
...
/* id - case dep., ia - case arr., ip - case pri. */
    ...
    if (!(flaglegalsquare ? legalsquare(id, ia, ip) : eval_ortho(id, ia, ip)))
        return false;

    p= e[sq_woo_from];
    if (nbpiece[p] && (*checkfunctions[abs(p)])(id, e[sq_woo_from], aux_whx))
	return true;
...
boolean woohefflibre(square to, square from) {   /* V3.55  TLi */
    ...
    for (pcheck= rex_wooz_ex ? transmpieces+1:transmpieces; *pcheck; pcheck++) {
        p= (CondFlag[biwoozles] ^ (col_woo == noir)) ? -*pcheck : *pcheck;
...
boolean scorpioncheck(square i, piece p, boolean (* evaluate)(square,square,square))   /* V2.63  NG */
{
	return (leapcheck(i, 1, 8, p, evaluate) ||
		rhopcheck(i, 1, 8, p, evaluate));
}

boolean dolphincheck(square i, piece p,
