...
square renrank(piece p, Flags pspec, square j, square i, couleur camp) {
    /* V3.45  TLi */
    square sq= (j / 24) & 1 ? rennormal (p, pspec, j, i, camp) :
			      renspiegel(p, pspec, j, i, camp);
...
{
	return  renfile(p, pspec, j, i, advers(camp));
} /* renspiegelfile */
	...
	if (camp == noir) {             /* captured white piece */
		if (is_pawn(p))
			...
			case roib:
					Result= 204;
					break;
			case cb:        if (cou == blanc)
						Result= 201;
					else
						Result= 206;
					break;
			case tb:        if (cou == blanc)
						Result= 207;
					else
						Result= 200;
					break;
...
square rendiametral(piece p, Flags pspec, square j, square i, couleur camp) {
    /* V3.44  TLi */
    return 575-rennormal(p, pspec, j, i, camp);
} /* rendiametral */

square renspiegel(piece p, Flags pspec, square j, square i, couleur camp)
{
	return  rennormal(p, pspec, j, i, advers(camp));
} /* renspiegel */
...
void genrn_cast(void) {    /* V3.55  TLi */
	...
	/* Note: 372 is square e8, 371 and 373 are d8 and f8 */
	if (((castling_flag[nbply]&0x07) > 0x04) &&
	/* then the king on e8 and at least one rook can castle !! */	/* V3.55  NG */
	    !echecc(noir)) {
		if ((castling_flag[nbply]&0x05) == 0x05 &&      /* 0-0 */
		    e[373] == vide && e[374] == vide) {
			e[372]= vide;
			e[373]= roin;
			rn= 373;
			if (!echecc(noir)) {
				...
				e[373]= vide;
				rn= 372;
				/* empile(372, 374, 374);      V3.55  TLi */
				empile(372, 374, maxsquare+2);       /* V3.55  TLi */
			} else {
				e[372]= roin;
				e[373]= vide;
				rn= 372;
		...
		if ((castling_flag[nbply]&0x06) == 0x06 &&      /* 0-0-0 */
		    e[371] == vide && e[370] == vide && e[369] == vide) {
			e[372]= vide;
			e[371]= roin;
			rn= 371;
			if (!echecc(noir)) {
				...
				e[371]= vide;
				rn= 372;
				/* empile(372, 370, 370);      V3.55  TLi */
				empile(372, 370, maxsquare+3);       /* V3.55  TLi */
			} else {
				e[372]= roin;
				e[371]= vide;
				rn= 372;
...
void static orig_gen_bl_piece(square z, piece p) { /* V3.71 TM */
	piece pp;
	...
	if (anymars) {                  /* V3.46  SE/TLi */
		...
		if (is_phantomchess) {  /* V3.47  NG */
			...
			flagactive= true;
			mren=(*marsrenai)(p,psp=spec[z],z,initsquare,blanc);
...
square next_latent_pawn(square s, couleur c) { /* V3.71 TM */
  piece pawn = c==blanc ? pb : pn;
  int delta = c==blanc ? -1 : 1;
  int i;
  if (s==initsquare)
  {
    i = 0;
...
{
  piece pprom;
  for (pprom = getprompiece[p]; pprom!=vide; pprom = getprompiece[pprom])
  {
#ifdef ASSERT
...
boolean jouecoup(void) {
	square  i,      /* case dep. */
		j,      /* case arr. */
		ip,     /* case pri. */
		cren= initsquare,   /* case ren. */     /* initialised !  V3.50  NG */
		_rb, _rn;
	piece   pp,     /* piece pri. */
		pj,     /* piece joue */
		pja,    /* piece joue arr. */
	...
	/* Orphans/refl. KK !!!! V3.0  TLi , now also for phantomchess  V3.47  NG - schoen krampfig */
   ...
   if (CondFlag[imitators])                /* V2.4d  TM */
	...
	switch (ip) {                /* V3.55  TLi */
		...
		CLEARFL(spec[i+3]);
		if (i == 204)
                	castling_flag[nbply]&= 0x27;    /* clear ke1, rh1 */
                else
			castling_flag[nbply]&= 0x72;    /* clear ke8, rh8 */
		break;
		...
		CLEARFL(spec[i-4]);
		if (i == 204)
			castling_flag[nbply]&= 0x17;    /* clear ke1, ra1 */
 		else
			castling_flag[nbply]&= 0x71;    /* clear ke8, ra8 */
                break;
	} /* switch (ip) */
				...
				if (!CondFlag[noiprom] && Iprom[nbply]) {   /* V2.4d  TM */
                    ...
                    if (   sbtype1 /* V3.71 TM */
                      ...
                      if (pp != vide && anyanticirce) {
						while ((cren= (*antirenai)(pja, pjspec, ip, i, advers(trait[nbply]))) != i &&
                               !LegalAntiCirceMove(cren, ip, i))  /* V3.62  TLi */
	...
	if (change_moving_piece) {              /* V3.1  TLi */
		if (CondFlag[degradierung] &&           /* V3.1  TLi */
		    !is_pawn(pj) &&
		    i != _rn && i != _rb &&
		    (trait[nbply] == noir ? ((j > haut-32) && (j<haut - 8)) :
					     ((j < bas+32) && (j>bas + 8)))) {
			if (pj < vide)
				pja= pn;
			else
		...
		if ((CondFlag[tibet] && pp != vide && pja != -pp &&        /* V2.90 TLi */
			(((trait[nbply] == noir) && (i != _rn)) ||
			    ((trait[nbply] == blanc) && CondFlag[dbltibet] && (i != _rb))))
		     ||
		     (CondFlag[andernach] && pp != vide && i != _rn && i != _rb)) {
			...
			if (TSTFLAG(pjspec, Black) && (!TSTFLAG(pjspec, White) || neutcoul == noir))
				pja= -pja;
		}

	        /* this seems to be misplaced -- it's checked every time TLi */
		/* therefore moved to this place !                 V3.60  NG */
	        if ( CondFlag[antiandernach] && pp == vide && i != _rn && i != _rb ) {
			...
			** pieces   V3.62  TLi */
                        ...
                        if (trait[nbply] == noir) {                                                             SETFLAG(pjspec, White);
                                pja= abs(pja);
                        } else {
                                SETFLAG(pjspec, Black);
                                pja= -abs(pja);
		...
		/* V3.33  TLi */
		if ((CondFlag[traitor] && trait[nbply] == noir &&        /* V3.1  TLi */
		      j < 296 && !TSTFLAG(pjspec, Neutral))
		    ||
		    (TSTFLAG(pjspec, Volage) && SquareCol(i) != SquareCol(j))
		    ||
		    (TSTFLAG(sq_spec[j], MagicSq) && _rn != i && _rb != i)) {
			CHANGECOLOR(pjspec);
			if (/* CondFlag[volage] && */ !CondFlag[hypervolage])
				CLRFLAG(pjspec, Volage);
			pja= -pja;
		...
		if (CondFlag[einstein] &&               /* V3.1  TLi */
			...
			pja= (pp == vide) ^ CondFlag[reveinstein] ?
			     dec_einstein(pja) : inc_einstein(pja);

	} /* change_moving_piece */

	if (i == _rb) {                          /* V2.1c  NG */
		rb= j;
		castling_flag[nbply]&= 0x37;    /* clear ke1 */
	}
	if (i == _rn) {
		rn= j;
		castling_flag[nbply]&= 0x73;    /* clear ke8 */
	}
	...
	if (pj == tb)
		if (i == 207)
			castling_flag[nbply]&= 0x67;
		else if (i == 200)
			castling_flag[nbply]&= 0x57;
	if (pj == tn)
		if (i == 375)
			castling_flag[nbply]&= 0x76;
		else if (i == 368)
			castling_flag[nbply]&= 0x75;
	if (j == 207)
		castling_flag[nbply]&= 0x67;
	else if (j == 200)
		castling_flag[nbply]&= 0x57;
		...
		if (castling_supported)
			if (pja == roib && j == 204)         /* white king new on e1 */
				castling_flag[nbply]|= 0x40;
			else if (pja == roin && j == 372)    /* black king new on e8 */
				castling_flag[nbply]|= 0x04;
			else if (pja == tb) { 
				if (j == 207)                /* white rook new on h1 */
					castling_flag[nbply]|= 0x10;
				else if (j == 200)           /* white rook new on a1 */
					castling_flag[nbply]|= 0x20;
			} else if (pja == tn) { 
				if (j == 375)                /* black rook new on h8 */
					castling_flag[nbply]|= 0x01;
				else if (j == 368)           /* black rook new on a8 */
					castling_flag[nbply]|= 0x02;
			}
...
#ifdef	NODEF		/* V3.62  TLi */
      	SETFLAG(pjspec, trait[nbply]==noir ? Black : White);
#endif
   ...
   if (TSTFLAG(pjspec, HalfNeutral) && TSTFLAG(pjspec, Neutral))   /* V3.57 SE */
	...
	if (jouegenre) {                                /* V2.4c  NG */
		...
		/* AntiCirce  V3.1  TLi */
		if (pp != vide && anyanticirce) {
			cren= (*antirenai)(pja, pjspec, ip, i, advers(traitnbply));
			e[j]= vide;
			spec[j]= 0;
			crenkam[nbply]= cren;
		...
		/* castling  V3.35  NG */
		if (castling_supported)
			if (pja == roib && cren == 204)         /* white king new on e1 */
				castling_flag[nbply]|= 0x40;
			else if (pja == roin && cren == 372)    /* black king new on e8 */
				castling_flag[nbply]|= 0x04;
			else if (pja == tb) { 
				if (cren == 207)                /* white rook new on h1 */
					castling_flag[nbply]|= 0x10;
				else if (cren == 200)           /* white rook new on a1 */
					castling_flag[nbply]|= 0x20;
			} else if (pja == tn) { 
				if (cren == 375)                /* black rook new on h8 */
					castling_flag[nbply]|= 0x01;
				else if (cren == 368)           /* black rook new on a8 */
					castling_flag[nbply]|= 0x02;
			}
		...
		if (CondFlag[sentinelles])                      /* V2.90 TLi */
			if (i > bas + 8 && i < haut - 8 && !is_pawn(pj))	/* V3.64  NG,TLi */
				if (SentPionNeutral) {
					if (TSTFLAG(pjspec, Neutral)) {
					...
					if (nbpiece[sentinelb] > max_pb || nbpiece[sentineln] > max_pn || nbpiece[sentinelb]+nbpiece[sentineln] > max_pt) {     /* V3.60 SE */
                                        /* rules for sentinelles + neutrals not yet written
                                           but it's very likely this logic will need to be refined */
						nbpiece[e[i]]--;
						e[i]= vide;
						spec[i]= 0;
					}
				...
				else if ((traitnbply == noir) ^ SentPionAdverse) {    /* V3.50 SE */
					if (nbpiece[sentineln] < max_pn && nbpiece[sentinelb]+nbpiece[sentineln] < max_pt  &&                     /* V3.60,3.63 SE */
							(!flagparasent ||  (nbpiece[sentineln] <= nbpiece[sentinelb]+(pp==sentinelb?1:0)))) {	/* V3.62 SE */
						nbpiece[e[i]= sentineln]++;
						SETFLAG(spec[i], Black);  /* V3.39  TLi */
						senti[nbply]= true;
					}
				}
				else if (nbpiece[sentinelb] < max_pb && nbpiece[sentinelb]+nbpiece[sentineln] < max_pt &&                        /* V3.603.63 SE */
						(!flagparasent ||  (nbpiece[sentinelb] <= nbpiece[sentineln]+(pp==sentineln?1:0)))) {	/* V3.62 SE */
					nbpiece[e[i]= sentinelb]++;
		...
		if (anycirce) {                 /* V2.4c  NG */
			...
			if (TSTFLAG(pjspec, Kamikaze) && (pp != vide)) {
				if (CondFlag[couscous])
					cren= (*circerenai)(pp, ppspec, ip, i, traitnbply);
				else
					cren= (*circerenai)(pja, pjspec, ip, i, advers(traitnbply));
				if (cren != j) {
					e[j]= vide;
					spec[j]= 0;
					if ((e[cren] == vide) &&  /* V3.1  TLi */
					    ! (CondFlag[contactgrid] && nogridcontact(cren))) { /* V2.5c  NG */
						crenkam[nbply]= cren;
						e[cren]= pja;
						spec[cren]= pjspec;
						if (rex_circe) {
            ...
            else if (anytraitor)
            {
               ...
               CHANGECOLOR(ppspec);
				...
				if (CondFlag[couscous])         /* V3.1  TLi */
					...
					cren= (*circerenai)(pren, ppspec, ip, i, traitnbply);

				if (!rex_circe && (flag_testlegality || CondFlag[brunner]) && /* V3.50 SE */
					 (ip == _rb || ip == _rn)) {
					 /* ordinary circe and (isardam, brunner or ohneschach)
					 ** it can happen that the king is captured while
					 ** testing the legality of a move.
					 ** Therefore prevent the king from being reborn. */
					 cren= initsquare;
				}
				...
				if ((e[cren] == vide || flagAssassin) &&          /* V2.4c  NG */
					...
					if (rex_circe) {    /* V3.33  TLi */
						...
						/* castling  V3.35  NG */
						if (castling_supported)
							if (pren == roib && cren == 204)        /* white king new on e1 */
								castling_flag[nbply]|= 0x40;
							else if (pren == roin && cren == 372)   /* black king new on e8 */
								castling_flag[nbply]|= 0x04;
					}
					/* castling  V3.35  NG */
					if (castling_supported)
					...
					if (anycirprom && is_pawn(pp) &&
					    PromSq(advers(traitnbply), cren)) {/* V3.39  TLi */
						/* captured white pawn on eighth rank: promotion ! */
						/* captured black pawn on first rank: promotion ! */
		...
		if (CondFlag[republican]) {         /* V3.50 SE */
			/* V3.53 Code involving flag_repub has no effect as it was included
			to ensure that if a side can place a mated K, it must. This is
			apparently not the rule, but code left in in case anybody wants alternative rule
			(return statement from jouecoup would need modifying) - SE */
			flag_repub= (super[nbply] < bas);
			if (trait[nbply]==blanc) {
				if (flag_writinglinesolution);      /* v3.53 SE */
				else
				if (!rn && !flag_dontaddk) {
					flag_dontaddk= true;
      ...
      if (CondFlag[arc])         /* V3.62 SE */
      {
         	square id, ia;
            id=i; ia=j;
         	if (id==275 || id==276 || id==299 || id==300 || ia==275 || ia==276 || ia==299 || ia==300)
            {
              Flags temp=spec[275];
              piece ptemp=e[275];
              e[275]= e[276];
...
/*   if (trait[nbply]==noir && !echecc(blanc)) return false;	*/	/* V3.62  SE */

	return (!jouetestgenre ||                      /* V3.50 SE */
		((!flag_testlegality) || pos_legal()) &&    /* V3.44, 3.51  SE/TLi */
		(!flagAssassin || (cren != rb && cren != rn)) &&        /* V3.50 SE */
		(!testdblmate || (rb!=initsquare && rn!=initsquare)) && /* V3.50 SE */
		(!CondFlag[patience] || PatienceB || patience_legal())  /* V3.50 SE */
					/* don't call patience_legal if TypeB as obs > vide ! */
	  /*	  &&	(!CondFlag[republican] || flag_repub) */  /* V3.50 SE, removed v3.53 SE - rule clarification  */
      && (!CondFlag[blackultraschachzwang] || trait[nbply]==blanc || echecc(blanc))
      && (!CondFlag[whiteultraschachzwang] || trait[nbply]==noir || echecc(noir))
				);
} /* end of jouecoup */
...
/*   pjspec_= spec[j];   V3.62  TLi */
	...
	** V3.22  TLi
	*/
	if (CondFlag[supercirce] && pp != vide || isapril[abs(pp)]) {       /* V3.1, 3.52  TLi, SE */
		nextsuper= super[nbply];
		while ((e[++nextsuper] != vide) && (nextsuper < haut));
#ifdef NODEF	/* V3.52  SE */
		while ((e[++super[nbply]] != vide) && (super[nbply] < haut));
#endif	/* NODEF */
	}

	if (CondFlag[republican]) {           /* V3.50 SE */
		square sq= super[nbply];
		if (sq <= haut) {
			e[sq]= vide;
			if (sq==rn) rn=initsquare;
			if (sq==rb) rb=initsquare;
		}
   }
  ...
  if (next_prom) {
	if ((pprom= norm_prom[nbply]) != vide) {
        ...
        if (   sbtype1
          ...
          if (pp != vide && anyanticirce) {       /* V3.1  TLi */
			...
			while (pprom != vide &&
			       (rencas= (*antirenai)(pprom, pjspec, ip, i, advers(trait[nbply]))) != i &&
                   !LegalAntiCirceMove(rencas, ip, i))	/* V3.62  TLi */
              pprom= getprompiece[pprom];
          }
        }
		...
		if ((pprom == vide) && TSTFLAG(PieSpExFlags, Chameleon) &&      /* V3.1  TLi */
	...
	if (pprom == vide) {
		...
		if ((pprom == vide) && !(!CondFlag[noiprom] && Iprom[nbply])) { /* V2.4d  TM */
			if (CondFlag[supercirce] && pp != vide || isapril[abs(pp)]) {     /* V3.52 SE */
...
{
	square cazz= ca[nbcou], sqren= sqrenais[nbply], renkam= crenkam[nbply];
...
boolean stip_circuitB(couleur camp)
{
	...
	return ((sqren != initsquare && (DiaRen(spec[sqren]) == sqren)) &&
		!echecc(camp));
}
...
{
/*	return (!echecc(advers(camp)) && !echecc(camp) && patt(camp));   */
...
boolean stip_castling(couleur camp) {
  unsigned char diff= castling_flag[nbply-1]-castling_flag[nbply];

  return (diff == 0x50 || diff == 0x60 || diff == 0x05 || diff == 0x06) &&
	 !echecc(camp);
} /* stip_castling */
