...
boolean rubiech(square i, square itest, piece p, /* echiquier */ smallint *e_ub, boolean (* evaluate)(square,square,square))      /* V1.7c  NG */
{
	numvec k;
	square j;
...
boolean rrfouech(square i, square itest, numvec k, piece p, smallint x, boolean (* evaluate)(square,square,square)) /* V2.1c  NG */
{
numvec k1;
piece p1;
square j;
...
boolean rcardech(square i, square itest, numvec k, piece p, smallint x, boolean (* evaluate)(square,square,square)) /* V2.1c  NG */
{
	numvec k1;
	piece p1;
	square j;
...
boolean feebechec(boolean (* evaluate)(square,square,square))     /* V1.5c  NG */
{
	piece   *pcheck;
...
boolean marsechecc(couleur camp, boolean (* evaluate)(square, square, square)) {
piece p;
square i,z;
int ii,jj;
Flags psp;
boolean ch;
	...
	/* in phantomchess the kings are not included, but with rex inclusif they are */
	if (!is_phantomchess || (e[z] != e[rb] && e[z] != e[rn]) || rex_phan)	/* V3.51, V3.62  NG */
	if (rightcolor(e[z],camp)) {
		more_ren=0;
		do {
			i= (*marsrenai)(p=e[z],psp=spec[z],z,initsquare,camp);
	    if ((e[i]==vide) || (i==z)) {
		e[z]=vide;
		e[i]=p;
		spec[i]=psp;
		ch=(*checkfunctions[(p > 0)?p:-p])(i, camp ? e[rn] : e[rb], evaluate);
		e[i]=vide;
		e[z]=p;
		spec[z]=psp;
		if (ch) return true;
...
static boolean orig_rnechec(boolean (* evaluate)(square,square,square)) /* V3.71 TM */
{
	/* detect, if black king is checked     */
	/* I didn't change this function, because it would be much (20% !) slower. V2.60  NG */
	numvec k;
	piece p;
	square sq;
	...
	if (rb) {                       /* V3.1  TLi */
		if (CondFlag[whrefl_king]) {    /* V3.00  TLi, V3.47  NG */
			piece   *ptrans;
			boolean flag = true;
			...
			if (!CondFlag[whtrans_king] || flag)      /* V3.05  TLi, V3.47  NG */
				for (k= 8; k > 0; k--)                                  /* V2.4c  NG */
					if (e[rn + vec[k]] == roib)
						if ((*evaluate)(rb, rn, rn))    /* V1.3c  NG, V3.02  TLi */
							imech(rb, rn);               /* V2.4d  TM */
		} else {
		if ( CondFlag[sting] && (*checkfunctions[sb])(rn, roib, evaluate) )	/* V3.63  NG */
			return true;
		for (k= 8; k > 0; k--)                                  /* V2.4c  NG */
			if (e[rn + vec[k]] == roib)
				if ((*evaluate)(rb, rn, rn))    /* V1.3c  NG, V3.02  TLi */
	...
	if (nbpiece[pb]) {
		if ((rn >= bas + 48) || CondFlag[parrain] ||
					CondFlag[einstein]) {
					/* V1.6c  NG ,  V3.02  TLi , V3.1  TLi */
			if (e[rn - 23] == pb)
				if ((*evaluate)(rn - 23, rn, rn))   /* V1.3c  NG, V3.02  TLi */
					imech(rn - 23, rn);     /* V2.4d  TM */
			if (e[rn - 25] == pb)
				if ((*evaluate)(rn - 25, rn, rn))   /* V1.3c  NG, V3.02  TLi */
					imech(rn - 25, rn);     /* V2.4d  TM */
			if (ep[nbply] && RN_[nbply] != rn) {            /* V3.45  TLi */
			    /* ep captures of royal pawns                                        */ 
			    /* ep[nbply] != initsquare --> a pawn has made a double/triple step. */
			    /* RN_[nbply] != rn --> the black king has moved                     */
			    square sq= ep[nbply];      /* square passed */
			    if (e[sq-23] == pb && (*evaluate)(sq-23, sq, rn)) imech(sq-23, sq);
			    if (e[sq-25] == pb && (*evaluate)(sq-25, sq, rn)) imech(sq-25, sq);
	...
	if (nbpiece[cb])
		for (k= 9; k <= 16; k++)
			if (e[rn + vec[k]] == cb)
				if ((*evaluate)(rn + vec[k], rn, rn)) /* V1.3c  NG, V3.02  TLi */
	...
	if (nbpiece[db] ||
		...
		for (k= 4; k > 0; k--) {                        /* V2.4c  NG */
			...
			if ((p == tb) || (p == db))
				if ((*evaluate)(sq, rn, rn))    /* V1.3c  NG, V3.02  TLi */
...
boolean (*rnechec)(boolean (* evaluate)(square,square,square)) = &orig_rnechec; /* V3.71 TM */
...
square sq;
	...
	if (rn) {                               /* V3.1  TLi */
		if (CondFlag[blrefl_king]) {    /* V3.00  TLi, V3.47  NG */
			piece   *ptrans;
			boolean flag= true;
			for (ptrans= transmpieces; *ptrans; ptrans++)
				if (nbpiece[*ptrans] &&
			...
			if (!CondFlag[bltrans_king] || flag)      /* V3.05  TLi, V3.47  NG */
				for (k= 8; k > 0; k--)                                  /* V2.4c  NG */
					if (e[rb + vec[k]] == roin)
						if ((*evaluate)(rn, rb, rb))    /* V1.3c  NG, V3.02  TLi */
							imech(rn, rb);                  /* V2.4d  TM */
		} else {
		if ( CondFlag[sting] && (*checkfunctions[sb])(rb, roin, evaluate) )	/* v3.63  NG */
			return true;
		for (k= 8; k > 0; k--)                                  /* V2.4c  NG */
			if (e[rb + vec[k]] == roin)
				if ((*evaluate)(rn, rb, rb))    /* V1.3c  NG, V3.02  TLi */
	...
	if (nbpiece[pn]) {
		if ((rb <= haut - 48) || CondFlag[parrain] ||
					...
					/* V1.6c  NG , V3.02  TLi , V3.1  TLi */
			if (e[rb + 23] == pn)
				if ((*evaluate)(rb + 23, rb, rb))       /* V1.3c  NG, V3.02  TLi */
					imech(rb + 23, rb);     /* V2.4d  TM */
			if (e[rb + 25] == pn)
				if ((*evaluate)(rb + 25, rb, rb))       /* V1.3c  NG, V3.02  TLi */
					imech(rb + 25, rb);     /* V2.4d  TM */
			if (ep[nbply] && RB_[nbply] != rb) {            /* V3.45  TLi */
			    /* ep captures of royal pawns                                        */
			    /* ep[nbply] != initsquare --> a pawn has made a double/triple step. */
			    /* RB_[nbply] != rb --> the white king has moved                     */
			    square sq= ep[nbply];      /* square passed */
			    if (e[sq+23] == pn && (*evaluate)(sq+23, sq, rb)) imech(sq+23, sq);
			    if (e[sq+25] == pn && (*evaluate)(sq+25, sq, rb)) imech(sq+25, sq);
			    if (ep2[nbply]) {           /* Einstein triple step */
				sq= ep2[nbply];
				if (e[sq+23] == pn && (*evaluate)(sq+23, sq, rb)) imech(sq+23, sq);
				if (e[sq+25] == pn && (*evaluate)(sq+25, sq, rb)) imech(sq+25, sq);
	...
	if (nbpiece[cn])
		for (k= 9; k <= 16; k++)
			if (e[rb + vec[k]] == cn)
				if ((*evaluate)(rb + vec[k], rb, rb))   /* V1.3c  NG, V3.02  TLi */
	...
	if (nbpiece[dn] ||
		...
		for (k= 4; k > 0; k--) {                        /* V2.4c  NG */
			...
			if ((p == tn) || (p == dn))
				if ((*evaluate)(sq, rb, rb))    /* V1.3c  NG, V3.02  TLi */
...
boolean (*rbechec)(boolean (* evaluate)(square,square,square)) = &orig_rbechec; /* V3.71 TM */


boolean rncircech(square id, square ia, square ip)
{
	if (id == (*circerenai)(e[rn], spec[rn], ip, id, blanc))     /* V3.55 SE */
		return false;
...
{
	if (id == (*circerenai)(e[rb], spec[rb], ip, id, noir))     /* V3.55 SE */
		return false;
	return (*eval_2)(id, ia, ip);           /* V2.90  NG */
}
...
{
	immrenroin= (*immunrenai)(e[rn], spec[rn], ip, id, blanc);    /* V3.55 SE */
...
{
	immrenroib= (*immunrenai)(e[rb], spec[rb], ip, id, noir);    /* V3.55 SE */
...
boolean echecc(couleur camp)
{
	if ((camp == blanc) ^ CondFlag[vogt]) {
		...
		if (rb == initsquare)
			return false;
		if (rex_circe)
			if (CondFlag[pwc] || e[(*circerenai)(e[rb], spec[rb], rb, initsquare, noir)] == vide)     /* V3.55 SE */
				return false;
		...
		if (rn == initsquare)
			return false;
		if (rex_circe)
			if (CondFlag[pwc] || e[(*circerenai)(e[rn], spec[rn], rn, initsquare, blanc)] == vide)     /* V3.55 SE */
				return false;
		if (TSTFLAG(PieSpExFlags,Neutral))
			initneutre(blanc);
		if (flagAssassin) {
			...
			for (bnp= boardnum; *bnp; bnp++)
				if ((p= e[*bnp]) && p<roin &&
					(*circerenai)(p,spec[*bnp],*bnp,initsquare,blanc)==rn) {
					square rn_sic = rn;
					rn = *bnp;
					flagAssassin=false;
					flag=rnechec(eval_black);
...
{
	if (TSTFLAG(spec[id], Paralyse) || (flaglegalsquare && !legalsquare(id, ia, ip)))
		return false;
...
#ifdef NODEF	/* moded to py.h, V3.62  TLi */
#define LegalAntiCirceMove (e[cren] == vide || (!AntiCirCheylan && cren == ip) \
					    || (cren == id))
#endif

boolean AntiCirceEch(square id, square ia, square ip, couleur camp) { /* V3.39  TLi */
	...
	square  cren= (*antirenai)(TSTFLAG(spec[id], Chameleon) ?
                            champiece(e[id]) : e[id], spec[id], ip, id, camp);
...
boolean check;
	...
	if (we_generate_exact)
		StdString("rbultra is called while we_generate_exact is set!\n");
...
/* boolean rnexactech(square id, square ia, square ip)  V3.22  TLi */
...
/**********  V3.12  TLi  begin  **********/
boolean skycharcheck(piece p, square king, square chp, square arr1, square arr2,
