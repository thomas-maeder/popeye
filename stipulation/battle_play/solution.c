 ...
 */
static slice_index alloc_solution_writer_slice(stip_length_type length,
                                               stip_length_type min_length)
{
  ...
  result = alloc_branch(STBattlePlaySolutionWriter,length,min_length);
...
static boolean solution_writer_prepend(slice_index si, slice_traversal *st)
{
...

  {
    ...
    slice_index const writer = alloc_solution_writer_slice(length,min_length);
...
static slice_operation const solution_writer_inserters[] =
{
...
void stip_insert_solution_writers(void)
{
  ...
  slice_traversal_init(&st,&solution_writer_inserters,0);
