...
static void generate_leg(square sq_departure, numvec zig, numvec zag)
{
...
static void generate_in_direction(square sq_departure, numvec zig, numvec zag)
{
  generate_leg(sq_departure,zig,zag);
  generate_leg(sq_departure,zig,-zag);
}
...
void spiralspringer_generate_moves(square sq_departure)
{
  ...
  for (k = vec_knight_start; k<=vec_knight_end; ++k)
    generate_in_direction(sq_departure,vec[k],vec[top-k]);
...
void diagonalspiralspringer_generate_moves(square sq_departure)
{
  ...
  {
    generate_in_direction(sq_departure,vec[k],vec[k+1]);
    generate_in_direction(sq_departure,vec[k+1],vec[k]);
  }
