	...
	while (True) {
		...
		for (ActLang= 0; ActLang<LangCount; ActLang++) {
			...
			if (GetUniqIndex(TokenCount,TokenTab,tok) == BeginProblem) {
				...
				VariantTypeTab= &(VariantTypeString[ActLang][0]);  /* v3.50 SE */
            ExtraCondTab= &(ExtraCondString[ActLang][0]);    /* V3.62 SE */
...
{
	/* We search the array PieNam, for an index, where it
	** matches the two characters a and b
	*/
	int indexx;
	char *ch;

	ch= PieceTab[2];
	for (indexx= 2; indexx<PieceCount; indexx++,ch+= sizeof(PieceChar))
		if (*ch == a && *(ch + 1) == b)
			return indexx;
	return 0;
}
...
static char *ParseFieldList(char *tok, PieNam Name, Flags Spec,
			    char echo)    /* V3.40  TLi */
{
	...
	while (True) {
		if (*tok && tok[1] && (Field=FieldNum(*tok,tok[1]))) {
			...
			if (echo) {
			    ...
			    /* LaTeX  V3.46  TLi */
			    sprintf(GlobalStr, "%s\\%c%s %c%c",
				    e[Field] == vide ? "+" : "",
				    TSTFLAG(Spec, Neutral) ? 'n' :
					  TSTFLAG(Spec, White) ? 'w' : 's',
				    Name > Bishop ? LaTeXPiecesAbbr[Name]
						  : LaTeXStdPie[Name],
			...
			spec[Field] = Spec;
			e[Field] = TSTFLAG(Spec, White) ? Name : - Name;   /* V3.33  TLi */
...
static char *ParseForsyth(void)
{
   ...
   while (sq && *tok)
   {
      ...
      else if (isalpha(*tok))
      {
         ...
         if (pc >= King)
         	sq= SetSquare(sq, pc, islower(InputLine[(tok++) - TokenLine]), &NeutralFlag);
         else tok++;                   /* error */
      }
      else if (*tok == '.')
      {
         ...
         if (pc >= King)
         	sq= SetSquare(sq, pc, islower(InputLine[(tok+1 - TokenLine)]), &NeutralFlag);
         tok += 3;
      }
      else if (*tok == '=')
      {
	...
	while (True) {
		...
		while ((ps= GetUniqIndex(PieSpCount,PieSpTab,tok)) >= -1) {
			if (ps == -1)
				...
				if (TmpFlg && (TmpFlg & BIT(ps)) && (TmpFlg != BIT(ps)))
...
static char *ParsStips(char *tok) {
	...
	*tok= '\0';
	...
	if (ReciStip[0] == 'z') {
	    ...
	    ReciAlphaEnd[2]= '\0';
	    if (!(ReciTargetField= FieldNum(ReciStip[1], ReciStip[2]))) {
	...
	if (strstr(ReciStip, "ep")) {
		 ReciStipulation= stip_ep;
	}
	else
	if (strstr(ReciStip, "ctr")) {
		 ReciStipulation= stip_circuitB;
	}
	else
	if (strstr(ReciStip, "<>r")) {
		 ReciStipulation= stip_exchangeB;
	}
	else
	if (strstr(ReciStip, "ct")) {
	...
	if (!(NonReciTargetField= FieldNum(tok[1], tok[2]))) {
	    IoErrorMsg(MissngFieldList, 0);
	    return (char *)0;
	}
...
static char *ParseStip(void)             /* H.D. 10.02.93 */
{
	...
	if (tok) {
		...
		if (!(SortFlag(Proof) && FlowFlag(Alternate))) {    /* V3.35  TLi, V3.42  NG */
...
static char *ReadSquares(smallint which) {
	...
	if (i != 0) {
		...
		case ReadNoCastlingSquares:	/* V3.55  NG */
		    switch (i) {
			case 200: no_castling&= 0x57;
				break;
			case 204: no_castling&= 0x37;
				break;
			case 207: no_castling&= 0x67;
				break;
			case 368: no_castling&= 0x75;
				break;
			case 372: no_castling&= 0x73;
				break;
			case 375: no_castling&= 0x76;
				break;
			default:
				break;
		    }
...
/* This is an ugly hack, but due to the new feature ReadFrischAuf, we need the returning */
/* of the token tok, and this leads to mistakes with other conditions that need reading  */
...
static char *ParseRex(boolean *rex, Cond what) {        /* V3.55  TLi */
        /* what == rexincl || rexexcl */
	char    *tok;

	*rex= (what==GetUniqIndex(CondCount, CondTab, tok= ReadNextTokStr()));
	if (*rex)
		tok= ReadNextTokStr();
	return tok;
} /* ParseRex */
...
static char *ParseVariant(boolean *type, int group)          /* SE, V3.50  NG */
{
	...
	*type= False;
	...
	if (VariantType >= -1) {
      ...
      else if (VariantType==PionNoirMaximum && group==gpSentinelles)      /* V3.60 SE */
      {
      	tok= ReadNextTokStr();
      	max_pn= atoi(tok);
         while (*tok && '0' <= *tok && *tok <= '9')
      ...
      else if (VariantType==PionBlancMaximum && group==gpSentinelles)     /* V3.60 SE */
      {
      	tok= ReadNextTokStr();
      	max_pb= atoi(tok);
         while (*tok && '0' <= *tok && *tok <= '9')
      ...
      else if (VariantType==PionTotalMaximum && group==gpSentinelles) /* V3.63 SE */
      { 
        tok= ReadNextTokStr();
        max_pt= atoi(tok);
         while (*tok && '0' <= *tok && *tok <= '9')
...
char *ReadChameleonCirceSequence(void) {  /* V3.45  NG */
    piece old_piece= vide, new_piece;
    char *tok, newpiece[3];
	...
	if (!new_piece) {
	    ...
	    old_piece= new_piece;
	    sprintf(newpiece, "%c%c", UPCASE(*tok), tok[1] == ' ' ? '\0' :
				      UPCASE(tok[1]));
...
/*	while ((indexx= GetUniqIndex(CondCount,CondTab,tok)) >= -1) { */
	while ((indexx= GetUniqIndex(CondCount,CondTab,tok)) >= -2) {  /* V3.62 SE */

      if (indexx == -2)                                           /* V3.62 SE */
      {
      	if ((indexx= GetUniqIndex(ExtraCondCount,ExtraCondTab,tok)) == -2)
         	break;
         switch (indexx)
         {
         	case maxi:
		...
		switch (indexx) {                       /* V2.70c  TLi */
			...
			case anti:
				...
				AntiCirType= AntiCirCheylan ? AntiCirTypeCheylan : AntiCirTypeCalvet;
				break;
			case antispiegel:
				tok= ParseVariant(&AntiCirCheylan, gpAntiCirce);
				AntiCirType= AntiCirCheylan ? AntiCirTypeCheylan : AntiCirTypeCalvet;
...
static char *ParseOpt(void)             /* H.D. 10.02.93 */
{
	...
	while ((indexx= GetUniqIndex(OptCount, OptTab, tok)) >= -1) {   /* V3.43  NG */
		if (indexx == -1) {
		...
		switch(indexx) {
		  case maxtime:         /* V3.53  NG */
			if ((maxsolvingtime= atoi(tok= ReadNextTokStr())) <= 0) {
		  ...
		  case maxsols:         /* V3.12  StHoe */
			if ((maxsolutions= atoi(tok= ReadNextTokStr())) <= 0) {
		  ...
		  case intelligent:         /* V3.53  TLi */
			if ((maxsol_per_matingpos= atoi(tok= ReadNextTokStr())) > 0) {
				/* we did use tok */
				break;
                        } else {
		  ...
		  case restart:                /* V3.44  TLi */
			if ((RestartNbr= atoi(tok= ReadNextTokStr())) <= 0) {
		  ...
		  case soltout:                       /* V2.90c  TLi */
			...
			break;                                          /* V2.1c  TLi */
		  case solessais:       /* for compatibility to older versions.  V2.90  NG */
			OptFlag[soltout]= True;
			maxdefen= 1;
			break;
		  case nontrivial:      /* V3.32  TLi */
			NonTrivialNumber=
                                strtol(tok= ReadNextTokStr(), &ptr, 10);
			if (tok == ptr) {
				IoErrorMsg(WrongInt, 0);
				return ReadNextTokStr();
			}
...
static char *ParseTwinMove(int indexx) {  /* V3.40  TLi */
    ...
    if (LaTeXout) {                 /* V3.47  NG */
        ...
        sprintf(GlobalStr, "\\%c%s %c%c",
	        TSTFLAG(spec[sq1], Neutral) ? 'n' :
	        TSTFLAG(spec[sq1], White) ? 'w' : 's',
	        abs(e[sq1]) > Bishop ? LaTeXPiecesAbbr[abs(e[sq1])]
				     : LaTeXStdPie[abs(e[sq1])],
	        'a'-8+sq1%24, '1'-8+sq1/24);
        strcat(ActTwin, GlobalStr);
    } /* LaTeXout */
	...
	WritePiece(e[sq2]);
		if (LaTeXout) {                 /* V3.47  NG */
		strcat(ActTwin, "$\\leftrightarrow$");
		sprintf(GlobalStr, "\\%c%s ",
		TSTFLAG(spec[sq2], Neutral) ? 'n' :
		TSTFLAG(spec[sq2], White) ? 'w' : 's',
		abs(e[sq2]) > Bishop ? LaTeXPiecesAbbr[abs(e[sq2])]
				     : LaTeXStdPie[abs(e[sq2])]);
		strcat(ActTwin, GlobalStr);
	}       /* LaTeXout */
    } else {
	StdString("-->");
	if (LaTeXout) {                 /* V3.52  NG */
	strcat(ActTwin, "$\\rightarrow$");
	}   /* LaTeXout */
    }
    WriteSquare(sq2);
	if (LaTeXout) {                 /* V3.47  NG */
...
static char *ParseTwinShift(void) {  /* V3.40  TLi */
	...
	if (LaTeXout) {                 /* V3.52  NG */
    /* LaTeX  V3.46  TLi */
    sprintf(GlobalStr, "%s %c%c$\\Rightarrow$%c%c", TwinTab[TwinShift],
    ...
    if (maxcol+diffcol > 15 || mincol+diffcol <  8 ||
	maxrank+diffrank > 15 || minrank+diffrank <  8) {
...
static char *ParseTwinRemove(void) {
	...
	sq= FieldNum(tok[0], tok[1]);
		...
		if (LaTeXout) {                 /* V3.47  NG */
			/* LaTeX */
			strcat(ActTwin, " --");
			strcat(ActTwin, TSTFLAG(spec[sq], Neutral) ? "\\n" :
					TSTFLAG(spec[sq], White) ? "\\w" : "\\s");
			strcat(ActTwin,
				abs(e[sq]) > Bishop  ?  LaTeXPiecesAbbr[abs(e[sq])] :
						  	LaTeXStdPie[abs(e[sq])]);
...
static char *ParseTwinSubstitute(void) {  /* V3.41  TLi */
	...
	sprintf(GlobalStr, "{\\w%s} $\\Rightarrow$ \\w%s",
	      p_old > Bishop ? LaTeXPiecesAbbr[p_old] : LaTeXStdPie[p_old],
	      p_new > Bishop ? LaTeXPiecesAbbr[p_new] : LaTeXStdPie[p_new]);
	strcat(ActTwin, GlobalStr);
	}       /* LaTeXout */
...
static char *ParseTwin(void) {
	 int indexx, i;
    char    *tok;
    boolean continued= False, TwinRead= False;
	...
	if (tk == TwinProblem || tk == NextProblem || tk == EndProblem) {
	    StdString("\n\n");
	if (LaTeXout) {                 /* V3.52  NG */
	    /* LaTeX  V3.46  TLi */
	    strcat(ActTwin, "\\\\");
	...
	if (!TwinRead) {                
		...
		if (LaTeXout) {                 /* V3.52  NG */
	        ...
	        sprintf(GlobalStr, "%c) ", TwinChar);
                ...
                sprintf(GlobalStr, "%c%d) ", (TwinChar-'a'-1)%('z'-'a') + 'a',
                                             (TwinChar-'a'-1)/('z'-'a'));
	    StdString(GlobalStr);
	    if (LaTeXout) {                 /* V3.52  NG */
	        strcat(ActTwin, GlobalStr);
	...
	if (LaTeXout) {                 /* V3.52  NG */
	    ...
	    if (OptFlag[appseul]) {
		char temp[5];
		sprintf(temp, " %c\\ra", tolower(*PieSpString[ActLang][White]));
		strcat(ActTwin, temp);
	    }
	}   /* LaTeXout */
	    break;
...
#ifdef NODEF	/* V3.64  NG
/* new condition: PromOnly */
char *ReadPromotionPieces(void) {  /* V3.43  NG */
    piece prom_piece;
    char *tok;
...
while (True) {
    switch (strlen(tok= ReadNextTokStr())) {
      case 1:
	prom_piece= GetPieNamIndex(*tok,' ');
	piece_read= True;		/* V3.57  NG */
	break;
      case 2:
	prom_piece= GetPieNamIndex(*tok,tok[1]);
	piece_read= True;		/* V3.57  NG */
	break;
      default:
...
void WriteConditions(int alignment) {  /* V3.40  TLi */
	...
	if (CondFlag[cond]) {
	    ...
	    if (cond == exact || cond == ultra)             /* V3.1  TLi */
		    continue;
	    if (cond == einstein &&
		  (CondFlag[reveinstein] || CondFlag[antieinstein]))  /* V3.50  TLi */
		    continue;
	    if ((cond == woozles && (CondFlag[biwoozles] ||    /* V3.55  TLi */
                                    CondFlag[heffalumps]) ||
	    ...
	    if (cond == refl_king && CondFlag[trans_king])
		    continue;
	    if (cond == whrefl_king && (CondFlag[refl_king] || CondFlag[whtrans_king]))
		    continue;
	    if (cond == blrefl_king && (CondFlag[refl_king] || CondFlag[bltrans_king]))
		    continue;
       ...
       if ((cond == blackultraschachzwang || cond == whiteultraschachzwang) && flagultraschachzwang)
       ...
       if (cond == sentinelles && flagparasent) {            /* V3.62 SE */
	    ...
	    if (cond == whconforsqu) {                 /* V3.20  NG */
		    square  i;
		    for (i= bas; i <= haut; i++)
			    if (TSTFLAG(sq_spec[i], WhConsForcedSq))  /* V3.20  TLi */
				    AddSquare(CondLine, i);
	    }
	    if (cond == blconforsqu) {                 /* V3.20  NG */
		    square  i;
		    for (i= bas; i <= haut; i++)
			    if (TSTFLAG(sq_spec[i], BlConsForcedSq))  /* V3.20  TLi */
				    AddSquare(CondLine, i);
	    }
		...
		(rex_immun && (cond == immun ||
			       cond == immunmalefique ||
			       cond == immundiagramm)) ||
		(rex_circe && (cond == circe ||
			       cond == circemalefique ||
			       cond == circediametral ||
		...
		if (SentPionAdverse) {        /* V3.50 SE */
			strcat(CondLine, "  ");
			strcat(CondLine, VariantTypeString[ActLang][PionAdverse]);
		}
		if (SentPionNeutral) {        /* V3.50 SE */
			strcat(CondLine, "  ");
			strcat(CondLine, VariantTypeString[ActLang][PionNeutral]);
		}
...
void WritePosition()
{
	...
	if (NonTrivialLength < enonce - 1)	/* changed V3.62  TLi */
	    sprintf(StipOptStr+strlen(StipOptStr), ";%d,%d", 
		    NonTrivialNumber, NonTrivialLength);
...
void LaTeXOpen(void) {                  /* V3.46  TLi */
    if (ActLang == German)
        fprintf(LaTeXFile, "\\documentstyle[german,diagram]{article}%%%s\n", VERSIONSTRING);
    else
        fprintf(LaTeXFile, "\\documentstyle[diagram]{article}%%%s\n", VERSIONSTRING);
...
void LaTeXEndDiagram(void) {               /* V3.46  TLi */
    ...
    if  (!(OptFlag[solmenaces] || OptFlag[solflights] || OptFlag[nontrivial]
          || (OptFlag[intelligent] && maxsol_per_matingpos)	/* V3.53  TLi */
	  || FlagTimeOut					/* V3.54  NG */
	  || FlagMaxSolsReached					/* V3.60  NG */
	  || (OptFlag[maxsols] && (solutions >= maxsolutions))	/* V3.54  NG */
...
void LaTeXBeginDiagram(void) {               /* V3.46  TLi */
    square *bnp;
    boolean firstpiece= true, fairypieces= false;
    smallint i;
    PieSpec sp;
	...
	*eol= '\0';
	...
	if (date != eol) {
		...
		break;
	      case '.':
		/* format is either day. month. year or day.-day. month. year */
		date--;
		tmp= date;
		while (strchr("0123456789", *(date-1)))
		    date--;
	...
	*eol= '\n';
	...
	if (e[*bnp] != vide) {
	    ...
	    fprintf(LaTeXFile, "%c%s%c%c",
		    ...
		    p <= Bishop ?
		      LaTeXStdPie[p] : LaTeXPiecesAbbr[p],
		    *bnp%24-200%24+'a',
		    *bnp/24-200/24+'1');
	    ...
	    if (p > Bishop && LaTeXPiecesAbbr[p][0] != '\0') {
		fairypieces= true;
	...
	fprintf(LaTeXFile, "*");
    if (OptFlag[appseul])
	fprintf(LaTeXFile, " %c\\ra", tolower(*PieSpString[ActLang][White]));
	...
	for (p= Bishop+1; p < PieceCount; p++)
		...
		for (q= Bishop+1; q < p; q++) {
			...
			fprintf(stderr, "+++ Warning: double representation '%s' for %s and %s\n",
			  LaTeXPiecesAbbr[q], LaTeXPiecesFull[p], LaTeXPiecesFull[q]); 
		    }
		}
		if (!firstpiece)
		    fprintf(LaTeXFile, "\\\\\n    ");
