...
static char *ParsePlay(char *tok, slice_index proxy)
{
  ...
  else if (strncmp("ser-hs",tok,6)==0)
  {
    ...
    if (tok!=0 && slices[proxy_leaf].u.pipe.next!=no_slice)
    {
      ...
      {
        slice_index const
            battle_branch = alloc_battle_branch(slack_length_battle,
                                                slack_length_battle);
        slice_index const hook = slice_insert_self_guards(battle_branch,
                                                          proxy_leaf);
  ...
  else
  {
    ...
    if (tok2!=0 && leaf!=no_slice)
    {
      ...
      if (result!=0)
      {
        if (length==slack_length_battle && min_length==slack_length_battle)
          pipe_link(proxy,leaf);
        else
        {
