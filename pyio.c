  ...
  for (gic = goalInputConfig; gic!=goalInputConfig+nr_goals; ++gic)
    if (strstr(tok,gic->inputText)==tok)
    {
      ...
      else if (gic->goal==goal_mate_or_stale)
      {
        ...
        pipe_link(proxy_mate,leaf_mate);
        pipe_link(proxy_stale,leaf_stale);
...
static char *ParseEnd(char *tok, slice_index proxy)
{
  ...
  if (strncmp("ser-dia",tok,7)==0
    ...
    switch (*tok)
    {
      ...
      default:
        tok = ParseGoal(tok,STLeafForced,proxy);
        break;
    }
 ...
 * @param proxy_leaf identifes proxy slice to append to series
 * @return input after stipulation if successful, 0 otherwise
 */
static char *ParseSerH(char *tok, slice_index proxy, slice_index proxy_leaf)
{
  stip_length_type length;
  stip_length_type min_length;
  char *result;
  ...
  if (result!=0)
  {
    ...
    slice_index const series
        = alloc_series_branch_next_other_starter(length+1,min_length,
                                                 proxy_help);
    pipe_set_successor(proxy_help,
                       alloc_help_branch(slack_length_help+1,
                                         slack_length_help+1,
                                         proxy_leaf ));
 ...
 * @return input after stipulation if successful, 0 otherwise
 */
static char *ParseSerS(char *tok, slice_index proxy, slice_index proxy_leaf)
{
  stip_length_type length;
  stip_length_type min_length;
  char *result;
  ...
  if (result!=0)
  {
    ...
    slice_index const hook = slice_insert_self_guards(defense_branch,
                                                      proxy_leaf);
    slice_index const series
        = alloc_series_branch_next_other_starter(length+1,min_length,hook);
    pipe_set_successor(proxy,series);
  }
...
static char *ParsePlay(char *tok, slice_index proxy)
{
      ...
      if (result!=0)
      {
        ...
        slice_index const branch
            = alloc_series_branch_next_other_starter(length,min_length,
                                                     proxy_leaf);
        pipe_link(mi,guard);
        pipe_link(guard,slices[proxy_leaf].u.pipe.next);
        pipe_link(proxy_leaf,mi);
        pipe_set_successor(proxy,branch);
        slices[slices[proxy_leaf].u.pipe.next].starter = White;
      }
    }
  ...
  else if (strncmp("semi-r",tok,6)==0)
  {
    ...
    if (tok!=0 && slices[proxy_avoided].u.pipe.next!=no_slice)
    {
      ...
      if (result!=0)
      {
        ...
        slice_index const
            hook = slice_insert_reflex_filters_semi(branch,proxy_avoided);
        pipe_set_successor(proxy,hook);
  ...
  else if (*tok=='s')
  {
    ...
    if (tok!=0 && slices[proxy_forced].u.pipe.next!=no_slice)
    {
      ...
      {
        slice_index const branch = alloc_battle_branch(length+1,min_length+1);
        slice_index const hook = slice_insert_self_guards(branch,proxy_forced);
        pipe_set_successor(proxy,hook);
        slices[slices[proxy_forced].u.pipe.next].starter = White;
      }
    }
  }
  ...
  else
  {
    ...
    if (tok2!=0 && leaf!=no_slice)
    {
      ...
      if (result!=0)
      {
        ...
        slice_index const hook = slice_insert_direct_guards(branch,proxy_leaf);
        pipe_set_successor(proxy,hook);
        slices[hook].starter = White;
      }
    }
  }
      ...
      max_length += slack_length_battle;
      {
        ...
        else
        {
          if (slices[slices[proxy_operand].u.pipe.next].type==STLeafForced)
            hook = slice_insert_self_guards(branch,proxy_operand);
          else
            hook = slice_insert_reflex_filters_semi(branch,proxy_operand);
        }
  ...
  if (min_length==0 || min_length==max_length)
  {
    ...
    if (tok!=0)
    {
      ...
      pipe_set_successor(proxy,
                         alloc_help_branch(max_length,min_length,proxy_to_op));
    }
  }
