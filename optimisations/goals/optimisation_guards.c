...
static void insert_goal_optimisation_filters(slice_index si, Goal goal)
{
  ...
  if (!state->is_optimised[si])
  {
    ...
    if (st->remaining<=slack_length_battle+2)
    {
      if (state->goal.type!=no_goal)
        insert_goal_optimisation_filters(si,state->goal);
...
{
  { STAttackMove,        &insert_goal_optimisation_guards_attack_move },
