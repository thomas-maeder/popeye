                              ...
                              char const *entry, char const *exit)
{
  ...
  state->nr_chars_written += protocol_fprintf(state->file,"%s",entry);
  ...
  state->nr_chars_written += protocol_fprintf(state->file,"%s",exit);
}
...
static void write_target(slice_index si, stip_structure_traversal *st)
{
  ...
  state->nr_chars_written += protocol_fprintf(state->file,"%s","z");
...
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","x");
}

static void write_castling(slice_index si, stip_structure_traversal *st)
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","00");
}
...
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","ep");
}
...
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","ct");
}
...
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","dia");
}

static void write_atob(slice_index si, stip_structure_traversal *st)
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","a=>b");
}

static void write_chess81(slice_index si, stip_structure_traversal *st)
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","c81");
}

static void write_kiss(slice_index si, stip_structure_traversal *st)
{
  state_type * const state = st->param;
  state->nr_chars_written += protocol_fprintf(state->file,"%s","k");
}
