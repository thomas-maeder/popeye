...
void InitCond(void) {     /* V3.40  TLi */
    ...
    wh_exact= wh_ultra= bl_exact= bl_ultra=
    anyclone= anycirprom= anycirce= anyimmun= anyanticirce= anytraitor=
    ...
    for (i= 0; i < CondCount; CondFlag[i++]= False);/* V2.70 TLi,V2.90 NG */

    for (i= maxply; i > 0; inum[i--]= 0);   /* V3.41  TLi */

    memset((char *) promonly, 0, sizeof(promonly));     /* V3.43, V3.44  NG */
...
void InitOpt(void) {  /* V3.40  TLi */
    ...
    for (i= 0; i < OptCount; OptFlag[i++]= False);  /* V2.70 TLi,V2.90 NG */
} /* InitOpt */
...
void InitBoard(void) {     /* V3.40  TLi */
    ...
    ActTitle[0]= ActAuthor[0]= ActOrigin[0]= ActTwin[0]= ActAward[0] = ActStip[0]= '\0';

    for (i= maxsquare - 1; i >= 0; i--) {           /* V2.60  NG, V3.22  TLi */
	    e[i]= e_ubi[i]= e_ubi_mad[i]= obs;          /* V3.29  NG */
	    spec[i]= BorderSpec;                    /* V3.33  TLi */
    }

    /* dummy squares for Messigny chess and castling -- must be empty, V3.55  TLi
    */
...
boolean leapcheck(square i, numvec kanf, numvec kend, piece p, boolean (* evaluate)(square,square,square))
...
/* detect "check" of a generalised rider-hopper p that runs up run_up squares
** and jumps jump squares. 0 indicates an potentially infinite run_up or jump.
...
boolean ridcheck(square i, numvec kanf, numvec kend, piece p, boolean (* evaluate)(square,square,square))
...
boolean marincheck(square i, numvec kanf, numvec kend, piece p,
                   boolean (* evaluate)(square,square,square)) {
	...
	for (k= kanf; k<= kend; k++)
		if (e[i - vec[k]] == vide) {
			...
			if (p1 == p &&
#ifdef NODEF
			    //(*evaluate)(j1, i, i - vec[k])    /* V3.02  TLi */
#endif
			    (*evaluate)(j1, i - vec[k], i))     /* V3.70  TLi */
				return true;
		}
	return false;
} /* marincheck */
...
boolean nogridcontact(square j)                 /* V2.4c  NG */
{
	...
	for (k= 8; k > 0; k--) {
		p= e[j1= j + vec[k]];
		if (p != vide && p != obs &&
		    /* gridnumber[j1] != gridnumber[j] */
		    GridNum(j1) != GridNum(j))
			return false;
	}
	return true;
}
...
/* id - case dep., ia - case arr., ip - case pri. */
{
	...
	if (CondFlag[haanerchess])
		e[id]= obs;
	else if (CondFlag[sentinelles] && id > bas + 8 && id < haut - 8 && !is_pawn(pj)) {	/* V3.64  NG,TLi */
		if ((pj <= roin) ^ SentPionAdverse) {            /* V3.50 SE */
			...
			if (nbpiece[sentinelb] < max_pb && nbpiece[sentinelb]+nbpiece[sentineln] < max_pt &&                         /* V3.60,3.63 SE */
							(!flagparasent ||  (nbpiece[sentinelb] <= nbpiece[sentineln]+(pp==sentineln?1:0))))	/* V3.62 SE */
				e[id]= sentinelb;
			else
				e[id]= vide;
	/* don't think any change as a result of Sentinelles PionNeutral is
   needed as piece specs not changed v3.50 SE */
	} else
		e[id]= vide;
...
/*      e[id] = CondFlag[haanerchess] ? obs : vide;     */
   ...
   else
   {
	...
	pren= pp= e[ip];
	/* the pieces can be reborn at the square where it has been captured.
	** For example, when it is taken by a locust or a similarly moving piece */
	e[ip]= vide;

	if (pp != vide && pp != obs) {     /* V1.6c , V3.05  NG , V3.1  TLi , V3.35  NG */
		if (anycirce && abs(pp) > roib) {       /* V3.1  TLi , V3.37  NG */
		/* correction by TLi V3.31
		** It still doesnot work with neutral pieces.
		** Eventually we must add the colour of the side making the move or
		** potentially giving the check to the argument list!
		*/
		if (anyclone && id != rn && id != rb)
			...
			pren= CondFlag[chamcirce] ? ChamCircePiece(pp) : pp;  /* V3.45  TLi */

		if (CondFlag[couscous]) 
			cr= (*circerenai)(pj, spec[id], ip, id, pp > vide ? blanc : noir);
		else
			cr= (*circerenai)(pren, spec[ip], ip, id, pp > vide ? noir : blanc);
	...
	/*****  TLi  V3.31  *****  end  *****/
		...
		if (abs(pj) == King && castling_supported) {
			...
			if (ip == maxsquare+3) {	/* 0-0-0 */	/* V3.63  NG */
				flag_castling= 2;
				e[ia + 1]= e[ia - 2];
				e[ia - 2]= vide;
			} 
	...
	if (CondFlag[contactgrid])              /* V2.4c  NG */
		...
		for (k= 8; k > 0; k--) {
			if ((p= e[ia + vec[k]]) != obs && p != vide) { /* 3.02 TLi */
...
boolean ooorphancheck(square i, piece porph, piece p, boolean (* evaluate)(square,square,square))
{
	...
	nbpiece[-p]--;
	e[i]= dummyb;
	for (bnp= boardnum; /* *bnp */ co < nrp; bnp++)
		if (e[*bnp] == p)
			olist[co++]= *bnp;
	for (k= 0; k < co; k++) {
		for (j= 0; j < co; e[olist[j]]= (k == j) ? p : dummyb, j++);
		if ((*checkfunctions[abs(porph)])(i, p, evaluate)) {
			for (j= 0; j < co; e[olist[j++]]= p);
			if (flag= ooorphancheck(olist[k], -porph, -p, evaluate))
				break;
		} else
			for (j= 0; j < co; e[olist[j++]]= p);
...
boolean fffriendcheck(square i, piece pfr, piece p,
        ...
        for (bnp= boardnum; cf < nrp; bnp++)
                if (e[*bnp] == p)
                        flist[cf++]= *bnp;
        for (k= 0; k < cf; k++) {
                for (j= 0; j < cf; e[flist[j]]= (k == j) ? p : dummyb, j++);
                if ((*checkfunctions[abs(pfr)])(i, p, evaluate)) {
                        for (j= 0; j < cf; e[flist[j++]]= p);
                        if (flag= fffriendcheck(flist[k], pfr, p, evaluate))
                                break;
                } else
                        for (j= 0; j < cf; e[flist[j++]]= p);
...
boolean friendcheck(square i, piece p,
                  boolean (* evaluate)(square,square,square)) { /* V3.65  TLi */
        piece   *pfr, cfr;
        boolean flag= false,    initialized= false;
        square  flist[63], *bnp;
        smallint k, j, cf= 0;
        ...
        for (pfr= orphanpieces; *pfr; pfr++) {
                ...
                if (nbpiece[cfr]) {
                        ...
                        for (k= 0; k < cf; k++) {
                                for (j= 0; j < cf;
                                      e[flist[j]]= (k == j) ? p : dummyb, j++);
                                if ((*checkfunctions[*pfr])(i, p, evaluate)) {
                                        for (j= 0; j < cf; e[flist[j++]]= p);
                                        if (flag= fffriendcheck(flist[k],
                                                              cfr, p, evaluate))
                                                break;
                                } else
                                        for (j= 0; j < cf; e[flist[j++]]= p);
