	...
	MaxLeng= 0;
        for (lan=0; lan<LangCount; lan++) {
                ...
                while (fgets(Line,sizeof(Line),MsgFile) != NULL) {
                        ...
                        sscanf(Line, "%d= ", &snum);
                        if (snum != StringCnt) {
                                fprintf(stderr, "Wrong Format of string %d\n",StringCnt);
                                exit(4);
                        }
                        sLine= strchr(Line, ' ')+1;
        ...
        fprintf(hd, "\n};\n");
	fprintf(hd, "#define STRINGCNT\t%d\n", StringCnt);
	fprintf(hd, "#define MAXLENG\t %d\n", MaxLeng);
	exit(0);
}
...

{
	...
	for (lan=0; lan<LangCount; lan++) {
		...
		*hpt= '\0';
		...
		hpt+= PutCardinal(sizeof(Head)+spt-StrBuf, hpt);/* Start of Offset */
