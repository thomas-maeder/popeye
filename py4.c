...
static int count_opponent_moves(void)
{
  ...
  attack(slices[temporary_hack_opponent_moves_counter[trait[nbply]]].u.fork.fork,length_unspecified);
    ...
    if (!k_cap && flagmummer[traitnbply])
    {
      ...
      if (is_new_longest_move)
      {
        if (!we_generate_exact)
        {
          ...
          is_this_move_legal = attack(slices[temporary_hack_maximummer_candidate_move_tester[trait[nbply]]].u.fork.fork,length_unspecified)==has_solution;
