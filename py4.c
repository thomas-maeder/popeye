...
short len_max(square id, square ia, square ip)  /* rewritten  V3.35  TLi */
{
    switch (ip) {     /* V3.55  TLi */
      case maxsquare+1:  /* Messigny chess type exchange --> no move, no length */
          return 0;
...
short len_min(square id, square ia, square ip)  /* rewritten  V3.35  TLi */
	...
	while (sic_nbc == nbcou) {
		if (jouecoup()) {  /* V3.44  SE/TLi */
			...
			*nbd= 0;
			...
			if (anymars) {                 /* V3.50 SE */
				boolean passive_sic= flagpassive,
					active_sic= flagactive,
					capture_sic= flagcapture;
				square marsid_sic= marsid;
				unsigned char more_ren_sic= more_ren;

				genmove(ad);
				flagpassive= passive_sic;
				flagactive= active_sic;
				flagcapture= capture_sic;
				marsid= marsid_sic;
				more_ren= more_ren_sic;
			} else
#endif
				genmove(ad);
			optimize= true;
...
/*  StdString("empile called: "); WriteSquare(id); StdString("-"); WriteSquare(ia); StdString("\n"); */
	if (id == ia)                     /* V2.1c  NG */
		return true;
	if (empilegenre) {              /* V2.51  NG */
		...
		if ( ( (CondFlag[nowhiteprom] && traitnbply==blanc && PromSq(blanc,ia)) ||
/* V3.64  NG */
		       (CondFlag[noblackprom] && traitnbply==noir && PromSq(noir,ia)) ) &&
		     is_pawn(e[id]) )
			return true;

		if (TSTFLAG(spec[id], Beamtet) ||           /* V3.53 TLi */
                    CondFlag[beamten] || CondFlag[central])      /* V1.4c  NG, V3.32  TLi , v3.50 SE */
			if (! soutenu(id, ia, ip))
				return true;
		...
		if (e[ip] != vide) {     /* V2.4c  NG */
			...
			if (CondFlag[nocapture] || 
			    (CondFlag[nowhcapture] && traitnbply==blanc) ||	/* V3.64  NG */
			    (CondFlag[noblcapture] && traitnbply==noir) ||	/* V3.64  NG */
...
/* the following is used for conditions where checktest is needed for
 * generation like follow my leader, maximummer.....            V2.90c  TLi */
		if (traitnbply == noir ? flagblackmummer : flagwhitemummer) {   /* V3.1  TLi */
...
/* printf("here\n"); */
		...
		if (stipulation == stip_target) {
			 if (ia != TargetField)
			return true;
		} else
		if (stipulation == stip_capture || stipulation == stip_steingewinn) {
			 if (e[ip] == vide)
			return true;
		} else
		if (stipulation == stip_castling) {   /* V3.35  TLi */
			 if (abs(e[id]) != King || abs(id-ia) != 2)
			return true;
		}
		 } else
		 if (totalortho && TSTFLAG(spec[id], Neutral) &&
		stipulation != stip_check) {
		/* Check if a mating move by a neutral piece can be retracted by
		** the opponent.
...
/*                        if (e[ip] != vide)    nK-bug ! */
			...
			if (e[id] != Pawn && e[id] != -Pawn || e[ip] == vide)
				 return true;
		} else
			 if (e[ip] == vide && e[id] != Pawn && e[id] != -Pawn)
			return true;
		 }
	}

lab1:   if (optimize) {                         /* V3.03  TLi */
		int     nbr;
		nbcou++;
		cd[nbcou]= id;
		ca[nbcou]= ia;
	...
	if (   cdkil != id || cakil != ia || flag_testlegality
...
/* generate rider-hopper moves from vec[kbeg] to vec[kend] */
  ...
  for (k= kbeg; k <= kend; k++) {
    if (run_up) {
      ...
      j2= i;
      if (run_up > 1) {
	/* The run up is longer. Check if there is an obstacle between
	** the hopper and the hurdle */
	...
	if (jump > 1) {
	  /* The jump up is a longer one. Check if there is an obstacle between
...
void grose(square i, int type, couleur camp)              /* V3.14  TLi, V3.63  NG */
{
	smallint k1, k2;
	square j;
	...
	for (k1= 9; k1 <= 16; k1++) {
		...
		if (type != _rrao && rightcolor(e[j], camp))    /* V3.14  TLi, V3.63  NG */
			testempile(i, j, j);
		j= i;
		k2= 8;
		do                                              /* V2.4d  TM */
...
/* generate rose lion/hopper moves on the single line k1 and direction k2
** V3.23  TLi
*/
{
square j = i;
numvec kinc = k2 ? -1 : 1;
	...
	if (j != i && (e[j] == vide || rightcolor(e[j], camp)))         /* V3.42  TLi */
	    testempile(i, j, j);
    }
} /* groselh */
...
void gmhop(square i, numvec kend, numvec kanf, smallint m, couleur camp)	/* V3.14  TLi, V3.62  NG */
{
	piece p;
	square j, j1;
	numvec k, k1;
	 ...
	 *      kend == 8, kanf == 5: bishop types (bishopmoose, bishopeagle, bishopsparrow)
	 *      kend == 4, kanf == 1: rook types   (rookmoose, rookeagle, rooksparrow)
	 */
...
void glocust(square i, numvec kbeg, numvec kend, couleur camp)  /* V3.53  TLi */
{
	piece p;
	square j;
	numvec k;
...
void gmarin(square i, numvec kbeg, numvec kend, couleur camp)   /* V3.14  TLi */
{
...
void gchin(square i, numvec kbeg, numvec kend, couleur camp) /* V2.60  NG, V3.14  TLi */
{
...
/* Non-Stop-Equihopper */
	...
	for (k1= 3; k1 >= 0; k1--)
		for (k2= 72; k2 >= 0; k2-= 24) {        /* V2.90  NG */
			...
			if (e[sautoir] != vide) {
				j= (sautoir<<1) - i;
				if ((e[j] == vide || rightcolor(e[j], camp))  /* V2.1c  NG, V3.14  TLi */
...
void gubi(square sq, square i, couleur camp)    /* V1.7c  NG, V3.14  TLi */
{
	numvec k;
	square j;
...
void grfou(square i, square in, numvec k, smallint x, couleur camp,
	   boolean (* generate)(square,square,square))   /* V2.1c  NG, V3.14  TLi */
{
	/* ATTENTION: if first call of x is 1 and boolnoedge[i] use     empile() for generate,    */
	/*                                                else  use testempile() for generate !!  */
	smallint k1;
	square j;
...
void gcard(square i, square in, numvec k, smallint x, couleur camp,
	   boolean (* generate)(square,square,square))   /* V2.1c  NG, V3.14  TLi */
{
	/* ATTENTION: if first call of x is 1 use     empile() for generate,    */
	/*                              else  use testempile() for generate !!  */
	smallint k1;
	square j;
...
void    grefc(square i, square i1, smallint x, couleur camp)    /* V2.1c  NG, V3.14  TLi */
{
	square j;
	numvec k;
	...
	for (k= 9; k <= 16; k++) {
		if (x) {
			...
			for (k= 9; k <= 16; k++)
				if ((e[j= i1 + vec[k]] == vide) || (rightcolor(e[j], camp)))    /* V3.14  TLi */
					testempile(i, j, j);
	}
} /* grefc */
...
/* Equihopper */
	...
	for (k= 17; k <= 40; k++) {      /* 2,4; 2,6; 4,6; */ /* V2.90  NG */
		if (   abs(e[i + vec[k]]) >= roib
		    && (e[j1= i + (vec[k]<<1)] == vide || rightcolor(e[j1], camp))
...
void gmaooa(square id, square pass, square ia1, square ia2, couleur camp)
{
	if (e[pass] == vide) {
...
square *bnp;                            /* V2.60  NG */
	...
	switch  (abs(p)) {
	case maob:      gmao(i, camp);                  /* V3.12  TLi */
			return;
	case paob:      gchin(i, 1, 4, camp);           /* V3.14  TLi */
			return;
	case leob:      gchin(i, 1, 8, camp);           /* V3.14  TLi */
			return;
	case vaob:      gchin(i, 5, 8, camp);           /* V3.14  TLi */
			return;
	case naob:      gchin(i, 9, 16, camp);          /* V3.62  TLi */
			return;
	case roseb:     grose(i, _rrider, camp);        /* V3.14  TLi, V3.63  NG */
			return;
	case nequib:    gnequi(i, camp);                /* V3.14  TLi */
			return;
	case locb:      glocust(i, 1, 8, camp);         /* V3.53  TLi */
			return;
        ...
        case rooklocustb:                               /* V3.53  TLi */
                        glocust(i, 1, 4, camp);
                        return;
	case kangoub:   gkang(i, camp);                 /* V1.5c  NG, V3.14  TLi */
			return;
	case csb:       for (k= 9; k <= 16; k++)        /* V2.1c  NG */
				gcs(i, vec[k], vec[25 - k], camp);      /* V3.14  TLi */
			return;
	case hamstb:    ghamst(i);                      /* V3.12  TLi */
			return;
	case ubib:      for (bnp= boardnum; *bnp; bnp++)        /* V1.7c  NG */
				e_ubi[*bnp]= e[*bnp];
			gubi(i, i, camp);               /* V3.14  TLi */
			return;
	case mooseb:    gmhop(i, 8, 1, 0, camp);        /* V2.1c  NG, V3.14  TLi, V3.62  NG */
			return;
	case eagleb:    gmhop(i, 8, 1, 1, camp);        /* V2.1c  NG, V3.14  TLi, V3.62  NG */
			return;
	case sparrb:    gmhop(i, 8, 1, 2, camp);        /* V2.1c  NG, V3.14  TLi, V3.62  NG */
			return;
        case margueriteb:                               /* V3.64
TLi */
                        gmhop(i, 8, 1, 0, camp);
                        gmhop(i, 8, 1, 1, camp);
                        gmhop(i, 8, 1, 2, camp);
                        ...
                        ghamst(i);
                        return;
	case archb:     /* if (boolnoedge[i])  V3.22  TLi */   /* V2.1c  NG */
			if (NoEdge(i))   /* V3.22  TLi */
				for (k= 5; k <= 8; k++)
					grfou(i, i, vec[k], 1, camp, empile);   /* V3.14  TLi */
			else
				for (k= 5; k <= 8; k++)
					grfou(i, i, vec[k], 1, camp, testempile);       /* V3.14  TLi */
			return;
	case reffoub:   for (k= 5; k <= 8; k++)         /* V2.1c  NG */
				grfou(i, i, vec[k], 4, camp, testempile);       /* V3.14  TLi */
			return;
	case cardb:     for (k= 5; k <= 8; k++)         /* V2.1c  NG */
				gcard(i, i, vec[k], 1, camp, empile);   /* V3.14  TLi */
			return;
	case dcsb:      for (k= 9; k <= 14; k++)        /* V2.1c  NG */
				gcs(i, vec[k], vec[23 - k], camp);      /* V3.14  TLi */
			for (k= 15; k <= 16; k++)       /* V2.1c  NG */
				gcs(i, vec[k], vec[27 - k], camp);      /* V3.14  TLi */
			return;
	case refcb:     grefc(i, i, 2, camp);          /* V2.1c  NG, V3.14  TLi */
			return;
	case equib:     gequi(i, camp);                /* V2.60  NG, V3.14  TLi */
			return;
	case catb:      gcat(i, camp);                 /* V3.14  NG */
			return;
	case sireneb:   gmarin(i, 1, 8, camp);          /* V3.14  TLi */
			return;
	case tritonb:   gmarin(i, 1, 4, camp);          /* V3.14  TLi */
			return;
	case nereidb:   gmarin(i, 5, 8, camp);          /* V3.14  TLi */
			return;
	case orphanb:   gorph(i, camp);                 /* V3.0  TLi, V3.14  NG */
			return;
        case friendb:   gfriend(i, camp);               /* V3.65  TLi */
                        return;
	case edgehb:    gedgeh(i, camp);                /* V3.14 TLi */
			return;
	case moab:      gmoa(i, camp);                  /* V3.12  TLi */
			return;
	case moaridb:   gemoarider(i, camp);
			return;
	case maoridb:   gemaorider(i, camp);
			return;
	case bscoutb:   for (k= 5; k <= 8; k++)        /* V3.05  NG */
				gcs(i, vec[k], vec[13 - k], camp);      /* V3.14  TLi */
			return;
	case gscoutb:   for (k= 4; k > 0; k--)         /* V3.05  NG */
				gcs(i, vec[k], vec[5 - k], camp);       /* V3.14  TLi */
			return;
	case skyllab:   geskylla(i, camp);              /* V3.12  TLi */
			return;
	case charybdisb:
	...
	case sb:        gerhop(i, 1, 8, camp);
			return;
	case lionb:     gelrhop(i, 1, 8, camp);
			return;
	case nsautb:    gerhop(i, 9, 16, camp);
			return;
	case camhopb:   gerhop(i, 33, 40, camp);
			return;
	case zebhopb:   gerhop(i, 25, 32, camp);
			return;
	case gnuhopb:   gerhop(i, 33, 40, camp);
			gerhop(i, 9, 16, camp);
			return;
	case tlionb:    gelrhop(i, 1, 4, camp);
			return;
	case flionb:    gelrhop(i, 5, 8, camp);
			return;
	case rookhopb:  gerhop(i, 1, 4, camp);
			return;
	case bishophopb:gerhop(i, 5, 8, camp);
			return;
	case contragrasb:
			gecrhop(i, 1, 8, camp);
			return;                         /* V3.29  NG */
	case roselionb: for (k= 9; k <= 16; k++) {
	...
	case rosehopperb:
			...
			return;
	case g2b:       gerhop2(i, 1, 8, camp);        /* V3.34  TLi */
			return;
	case g3b:       gerhop3(i, 1, 8, camp);        /* V3.35  TLi */
			return;
	case khb:       geshop(i, 1, 8, camp);         /* V3.44  SE */
			return;
	case doublegb:  gdoubleg(i, camp);             /* V3.44  SE */
			return;
	case orixb:     gorix(i, camp);                /* V3.44  NG */
			return;
	case gralb:    if (camp==blanc)                /* V3.46  NG */
				gebleap(i, 65, 68);    /* alfilb */
			else
				genleap(i, 65, 68);    	/* alfiln */
			gerhop(i, 1, 4, camp);          /* rookhopper */
	    		return;
	case rookmooseb:    gmhop(i, 4, 1, 0, camp);    /* V3.62  NG */
			return;
	case rookeagleb:    gmhop(i, 4, 1, 1, camp);    /* V3.62  NG */
			return;
	case rooksparrb:    gmhop(i, 4, 1, 2, camp);    /* V3.62  NG */
			return;
	case bishopmooseb:    gmhop(i, 8, 5, 0, camp);    /* V3.62  NG */
			return;
	case bishopeagleb:    gmhop(i, 8, 5, 1, camp);    /* V3.62  NG */
			return;
	case bishopsparrb:    gmhop(i, 8, 5, 2, camp);    /* V3.62  NG */
			return;
	case raob:      for (k= 9; k <= 16; k++) {	/* V3.63  NG */
			     groselh(i, k, 0, _rrao, camp);
			     groselh(i, k, 8, _rrao, camp);
			}
			grose(i, _rrao, camp);
			return;
	case scorpionb: if (camp==blanc)                /* V3.63  NG */
				gebleap(i, 1, 8);       /* ekingb */
			else
				genleap(i, 1, 8);    	/* ekingn */
			gerhop(i, 1, 8, camp);		/* grashopper */
			return;
        case nrlionb:   gelrhop(i, 9, 16, camp);        /* V3.64  TLi */
                        return;
        case mrlionb:   gemaoriderlion(i, camp);        /* V3.64  TLi */
                        return;
        case molionb:   gemoariderlion(i, camp);        /* V3.64  TLi */
                        return;
        }
} /* gfeerrest */
...
square  j;
	...
	for (j= i + dir; e[j] == vide; j+= dir)
		empile(i, j, j);
} /* end of gen_sp_nocaptures */

void    gen_sp_captures(square i, numvec dir, couleur camp)
/* generates capturing moves of a super pawn of colour camp in direction dir.
*/
{
square  j;
piece   p;
	...
	if (rightcolor(p, camp))
		empile(i, j, j);
} /* end of gen_sp_captures */

void gfeerblanc(square i, piece p)
{
	testdebut= nbcou;               /* V3.00  NG */
	switch(p) {
	case nb:        gebrid(i, 9, 16);
			return;
	case zb:        gebleap(i, 25, 32);
			return;
	case chb:       gebleap(i, 33, 40);
			return;
	case gib:       gebleap(i, 41, 48);
			return;
	case rccinqb:   gebleap(i, 69, 80);
			return;
	case bub:       gebleap(i, 49, 60);
			return;
	case vizirb:    gebleap(i, 1, 4);
			return;
	case alfilb:    gebleap(i, 65, 68);
			return;
	case fersb:     gebleap(i, 5, 8);
			return;
	case dabb:      gebleap(i, 61, 64);
			return;
	case pbb:       genpbb(i);
			return;
	case amazb:     gebrid(i, 1, 8);                /* V1.4c  NG */
			gebleap(i, 9, 16);
			return;
	case impb:      gebrid(i, 1, 4);                /* V1.4c  NG */
			gebleap(i, 9, 16);
			return;
	case princb:    gebrid(i, 5, 8);                /* V1.4c  NG */
			gebleap(i, 9, 16);
			return;
	case gnoub:     gebleap(i, 33, 40);             /* V1.4c  NG */
			gebleap(i, 9, 16);
			return;
	case antilb:    gebleap(i, 49, 56);             /* V1.5c  NG */
			return;
	case ecurb:     gebleap(i, 61, 68);             /* V1.5c  NG */
			gebleap(i, 9, 16);
			return;
	case waranb:    gebrid(i, 9, 16);               /* V1.5c  NG */
			gebrid(i, 1, 4);
			return;
	case dragonb:   genpb(i);                       /* V1.5c  NG */
			gebleap(i, 9, 16);
			return;
	case camridb:   gebrid(i, 33, 40);              /* V2.1c  NG */
			return;
	case zebridb:   gebrid(i, 25, 32);              /* V2.1c  NG */
			return;
	case gnuridb:   gebrid(i, 33, 40);              /* V2.1c  NG */
			gebrid(i, 9, 16);
			return;
	case bspawnb:   gen_sp_nocaptures(i, 23);       /* V3.22  TLi */
			gen_sp_nocaptures(i, 25);
			gen_sp_captures(i, 24, blanc);
			return;
	case spawnb:    gen_sp_nocaptures(i, 24);       /* V3.22  TLi */
			gen_sp_captures(i, 23, blanc);
			gen_sp_captures(i, 25, blanc);
			return;
	case rhuntb:    gebrid(i, 2, 2);        /* V3.03  TLi */
			gebrid(i, 7, 8);
			return;
	case bhuntb:    gebrid(i, 4, 4);        /* V3.03  TLi */
			gebrid(i, 5, 6);
			return;
	case ekingb:    gebleap(i, 1, 8);       /* V3.05  NG */
			return;
	case okapib:    gebleap(i, 17, 32);     /* V3.32  TLi */
			return;
	case leap37b:   gebleap(i, 81, 88);     /* V3.34  TLi */
			return;
	case leap16b:   gebleap(i, 89, 96);     /* V3.38  NG */
			return;
	case leap24b:   gebleap(i, 97, 104);    /* V3.42  TLi */
			return;
	case leap35b:   gebleap(i, 105, 112);   /* V3.42  TLi */
			return;
	case leap15b:   gebleap(i, 113, 120);   /* V3.46  NG */
			return;
	case leap25b:   gebleap(i, 121, 128);   /* V3.46  NG */
			return;
	case vizridb:   gebrid(i, 1, 4);        /* V3.54  NG */
			return;
	case fersridb:  gebrid(i, 5, 8);        /* V3.54  NG */
			return;
	case bisonb:    gebleap(i, 25, 40);     /* V3.60  TLi */
			return;
	case elephantb: gebrid(i, 1, 16);       /* V3.62  TLi */
			return;
	case leap36b:   gebleap(i, 129, 136);   /* V3.64  TLi */
			return;
	default:        gfeerrest(i, p, blanc); /* V3.14  TLi */
	} /* of switch */
}
...
void gfeernoir(square i, piece p)
{
	testdebut= nbcou;               /* V3.00  NG */
	switch(p) {
	case nn:        genrid(i, 9, 16);
			return;
	case zn:        genleap(i, 25, 32);
			return;
	case chn:       genleap(i, 33, 40);
			return;
	case gin:       genleap(i, 41, 48);
			return;
	case rccinqn:   genleap(i, 69, 80);
			return;
	case bun:       genleap(i, 49, 60);
			return;
	case vizirn:    genleap(i, 1, 4);
			return;
	case alfiln:    genleap(i, 65, 68);
			return;
	case fersn:     genleap(i, 5, 8);
			return;
	case dabn:      genleap(i, 61, 64);
			return;
	case pbn:       genpbn(i);
			return;
	case amazn:     genrid(i, 1, 8);                /* V1.4c  NG */
			genleap(i, 9, 16);
			return;
	case impn:      genrid(i, 1, 4);                /* V1.4c  NG */
			genleap(i, 9, 16);
			return;
	case princn:    genrid(i, 5, 8);                /* V1.4c  NG */
			genleap(i, 9, 16);
			return;
	case gnoun:     genleap(i, 33, 40);             /* V1.4c  NG */
			genleap(i, 9, 16);
			return;
	case antiln:    genleap(i, 49, 56);             /* V1.5c  NG */
			return;
	case ecurn:     genleap(i, 61, 68);             /* V1.5c  NG */
			genleap(i, 9, 16);
			return;
	case warann:    genrid(i, 9, 16);               /* V1.5c  NG */
			genrid(i, 1, 4);                /* V1.5c  NG */
			return;
	case dragonn:   genpn(i);                       /* V1.5c  NG */
			genleap(i, 9, 16);
			return;
	case camridn:   genrid(i, 33, 40);              /* V2.1c  NG */
			return;
	case zebridn:   genrid(i, 25, 32);              /* V2.1c  NG */
			return;
	case gnuridn:   genrid(i, 33, 40);              /* V2.1c  NG */
			genrid(i, 9, 16);
			return;
	case bspawnn:   gen_sp_nocaptures(i, -23);      /* V3.22  TLi */
			gen_sp_nocaptures(i, -25);
			gen_sp_captures(i, -24, noir);
			return;
	case spawnn:    gen_sp_nocaptures(i, -24);      /* V3.22  TLi */
			gen_sp_captures(i, -23, noir);
			gen_sp_captures(i, -25, noir);
			return;
	case rhuntn:    genrid(i, 2, 2);        /* V3.03  TLi, V3.04  NG */
			genrid(i, 7, 8);
			return;
	case bhuntn:    genrid(i, 4, 4);        /* V3.03  TLi, V3.04  NG */
			genrid(i, 5, 6);
			return;
	case ekingn:    genleap(i, 1, 8);       /* V3.05  NG */
			return;
	case okapin:    genleap(i, 17, 32);     /* V3.32  TLi */
			return;
	case leap37n:   genleap(i, 81, 88);     /* V3.34  TLi */
			return;
	case leap16n:   genleap(i, 89, 96);     /* V3.34  TLi */
			return;
	case leap24n:   genleap(i, 97, 104);    /* V3.42  TLi */
			return;
	case leap35n:   genleap(i, 105, 112);   /* V3.42  TLi */
			return;
	case leap15n:   genleap(i, 113, 120);     /* V3.46  NG */
			return;
	case leap25n:   genleap(i, 121, 128);     /* V3.46  NG */
			return;
	case vizridn:   genrid(i, 1, 4);        /* V3.54  NG */
			return;
	case fersridn:  genrid(i, 5, 8);        /* V3.54  NG */
			return;
	case bisonn:    genleap(i, 25, 40);     /* V3.60  TLi */
			return;
	case elephantn: genrid(i, 1, 16);       /* V3.62  TLi */
			return;
	case leap36n:   genleap(i, 129, 136);   /* V3.64  TLi */
			return;
	default:        gfeerrest(i, p, noir);  /* V3.14  TLi */
	} /* of switch */
} /* end of gfeernoir */
...
void genrb_cast(void) {   /* V3.55  TLi */
	...
	/* then the king on e1 and at least one rook can castle !! */	/* V3.55  NG */
	    !echecc(blanc)) {
		if ((castling_flag[nbply]&0x50) == 0x50 &&      /* 0-0 */
			...
			if (!echecc(blanc)) {
				...
				e[205]= vide;
				rb= 204;
				/* empile(204, 206, 206);    V3.55  TLi */
				empile(204, 206, maxsquare+2);     /* V3.55  TLi */
			} else {
				e[204]= roib;
				e[205]= vide;
				rb= 204;
		...
		if ((castling_flag[nbply]&0x60) == 0x60 &&      /* 0-0-0 */
			...
			if (!echecc(blanc)) {
				...
				e[203]= vide;
				rb= 204;
				/* empile(204, 202, 202);    V3.55  TLi */
				empile(204, 202, maxsquare+3);     /* V3.55  TLi */
			} else {
				e[204]= roib;
				e[203]= vide;
				rb= 204;
...
void gen_wh_piece_aux(square z, piece p) {              /* V3.46  SE/TLi */
	...
	case pb:        genpb(z);
			break;
	case cb:        gebleap(z, 9, 16);
			break;
	case tb:        gebrid(z, 1, 4);        /* V2.60  NG */
			break;
	case db:        gebrid(z, 1, 8);        /* V2.60  NG */
			break;
	case fb:        gebrid(z, 5, 8);        /* V2.60  NG */
			break;
	default:        gfeerblanc(z, p);
			break;
	}
} /* gen_wh_piece_aux */
...
static void orig_gen_wh_piece(square z, piece p) { /* V3.71 TM */
	...
	if (CondFlag[messigny] && !(rb == z && rex_mess_ex)) {   /* V3.55  TLi */
		square *bnp;
...
} /* singleboxtype3_gen_wh_piece */

void (*gen_wh_piece)(square z, piece p) = &orig_gen_wh_piece; /* V3.71 TM */
...
boolean IsABattery(square KingSquare, square FrontSquare, numvec Direction,
		   ColourSpec ColourMovingPiece, piece BackPiece1, piece BackPiece2) {
    square sq;
    piece p;
	...
	** battery piece? */
	...
	if (p < vide)
	    p= -p;
	if ((p == BackPiece1 || p == BackPiece2) && TSTFLAG(spec[sq], ColourMovingPiece))
	    /* So, it is a battery. */
	    return true;
    }
    return false;
...
void GenMatingPawn(square z, square king, ColourSpec ColourMovingPiece) {
    boolean Battery = false;
    square j;
    numvec k;

    if ((k= CheckDirBishop[king-z]) != 0)
	Battery= IsABattery(king, z, k, ColourMovingPiece, Bishop, Queen);
    else
    if ((k= CheckDirRook[king-z]) != 0)
	Battery= IsABattery(king, z, k, ColourMovingPiece, Rook, Queen);
    /* if the pawn is not the front piece of a battery reset k, otherwise
    ** normalise it to be positiv. This is necessary to avoid moves along the
    ** battery line subsequently.
    */
    if (Battery) {
	if (k < 0)
	    k= -k;
	...
	if (z < bas + 24) {
	    ...
	    /* not first rank */
	    /* ep captures */
	    if ((j = ep[nbply-1]) != initsquare && trait[nbply-1] != trait[nbply])
		if (z+25 == j || z+23 == j)
		...
		j= z+24;
		if (e[j] == vide) {
		    if (Battery || j+23 == king || j+25 == king ||
			(PromSq(blanc, j) && (CheckDirQueen[king-j] || CheckDirKnight[king-j])))
			empile(z, j, j);
		    /* double step */
		    if (z < bas+32) {
			j+= 24;
			if (e[j] == vide && (Battery || j+23 == king || j+25 == king))
			    empile(z, j, j);
		    }
		}
	    }
	...
	if (z > haut - 24) {
		...
		if (e[j] == vide) {
		    if (Battery || j-23 == king || j-25 == king ||
			(PromSq(noir, j) && (CheckDirQueen[king-j] || CheckDirKnight[king-j])))
			empile(z, j, j);
		    /* double step */
		    if (z > haut-32) {
			j-= 24;
...
void GenMatingKing(square z, square king, ColourSpec ColourMovingPiece) {
    square sq;
    numvec  k, k2;
	...
	/* check if the king is the front piece of a battery that can fire */
	if ((k = CheckDirBishop[king-z]) != 0) {
	    Generate= IsABattery(king, z, k, ColourMovingPiece, Bishop, Queen);
	} else
	if ((k = CheckDirRook[king-z]) != 0) {
	    Generate= IsABattery(king, z, k, ColourMovingPiece, Rook, Queen);
	}
...
void GenMatingKnight(square z, square king, ColourSpec ColourMovingPiece) {
    square sq;
    numvec  k;
    boolean Generate = false;
    ColourSpec ColourCapturedPiece = OppsiteColour(ColourMovingPiece);

    /* check if the knight is the front piece of a battery that can fire */
    if ((k = CheckDirBishop[king-z]) != 0) {
	Generate= IsABattery(king, z, k, ColourMovingPiece, Bishop, Queen);
    } else
    if ((k = CheckDirRook[king-z]) != 0) {
...
void GenMatingRook(square z, square king, ColourSpec ColourMovingPiece) {
    square sq, sq2;
    numvec  k, k2;
    ColourSpec ColourCapturedPiece = OppsiteColour(ColourMovingPiece);
	...
	if ((k2 = CheckDirRook[king-z])) {
		...
		if (move_diff_code[abs(sq-king)] < OriginalDistance) {
		    ...
		    while (!(k2= CheckDirRook[king-sq]) && e[sq] == vide)
			sq+= vec[k];
...
void GenMatingQueen(square z, square king, ColourSpec ColourMovingPiece) {
    square sq, sq2;
    numvec  k, k2;
    ColourSpec ColourCapturedPiece = OppsiteColour(ColourMovingPiece);
...
void GenMatingBishop(square z, square king, ColourSpec ColourMovingPiece) {
    square sq, sq2;
    numvec  k, k2;
    ColourSpec ColourCapturedPiece = OppsiteColour(ColourMovingPiece);

    /* check if the bishop is the front piece of a battery that can fire */
    if (((k = CheckDirRook[king-z]) != 0) &&
	...
	if ((k2 = CheckDirBishop[king-z])) {
		...
		if (move_diff_code[abs(sq-king)] < OriginalDistance) {
		    ...
		    while (!(k2= CheckDirBishop[king-sq]) && e[sq] == vide)
			sq+= vec[k];
		    /* We are at the end of the line or in checking distance */
		    if (k2 == 0)
			continue;
...
void GenMatingMove(couleur camp) {                /* V3.33  TLi */
	...
	for (i= 8; i > 0; i--, z+= 16)          /* V2.90  NG */
	for (j= 8; j > 0; j--, z++) {           /* V2.90  NG */
	    if ((p = e[z]) != vide && TSTFLAG(spec[z], ColourMovingPiece))
		switch(abs(p)) {
		    case King:      GenMatingKing(z, OpponentsKing, ColourMovingPiece);
				    break;
		    case Pawn:      GenMatingPawn(z, OpponentsKing, ColourMovingPiece);
				    break;
		    case Knight:    GenMatingKnight(z, OpponentsKing, ColourMovingPiece);
				    break;
		    case Rook:      GenMatingRook(z, OpponentsKing, ColourMovingPiece);
				    break;
		    case Queen:     GenMatingQueen(z, OpponentsKing, ColourMovingPiece);
				    break;
		    case Bishop:    GenMatingBishop(z, OpponentsKing, ColourMovingPiece);
				    break;
		}
	}
	finkiller();
	...
	if (FlagMoveOrientatedStip) {
	    ...
	    if (stipulation == stip_castling) {
		if (camp == blanc ?
			 ((castling_flag[nbply]&0x70) < 0x50) :
			 ((castling_flag[nbply]&0x07) < 0x05)) {
		    nextply();
		    return;
		}
	    }
	    /* FlagGenMatingMove= True; */
            FlagGenMatingMove= ! (camp == blanc ? flagwhitemummer
                                                : flagblackmummer);
	}
	genmove(camp);
    }
...
void gedgeh(square i, couleur camp)     /* V3.14  TLi */
{
	...
	j= i;
	while (e[j+= vec[k]] == vide)
	    /* if (boolnoedge[j] != boolnoedge[i])     V3.22  TLi */
...
/* generates move of a pawn of colour camp on i capturing a piece on j */
{
  if (rightcolor(e[j], camp)) {
    /* normal capture */
    empile(i, j, j);
	...
	(trait[nbply - 1] != camp)) {  /* the opponent moved before */
		if (nbply == 2)        /* ep.-key  standard pawn  V3.37  NG */
			if (camp == blanc)
				ca[repere[2]]= j - 24;
			else
				ca[repere[2]]= j + 24;
	if (rightcolor(e[ca[repere[nbply]]], camp)) /* the pawn has the right color */
...
/* generates moves of a pawn in direction dir where steps single steps
** are possible.
*/
{
square  j= i;
