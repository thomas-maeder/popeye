...
#define SetIndex(s, f) (s = (Flags)(s&((1<<DiaCirce)-1)) + (f<<DiaCirce))
#define GetIndex(s)       ((smallint)((s >> DiaCirce)))
...
boolean impact(square bk, piece p, square sq) {
	 ...
	 if (guards(bk, p, sq))
		  if (stipulation == stip_stale) {
				return (p == Queen) && (move_diff_code[abs(bk-sq)] == 4);
		  } else
				return true;
#endif
...
smallint FroTo(piece f_p, square f_sq, piece t_p, square t_sq, boolean genchk) {
      ...
      case Pawn:
        if (f_p == t_p) {
            ...
            if (f_p < vide) {
                /* black pawn */
                if (diffrow < diffcol)
                    /* if diffrow <= 0 then this test is true, since diffcol is
						  ** always positive */
						  return maxply+1;
            ...
            /* promotion */
            smallint minmoves= maxply+1, curmoves;
            square v_sq, start= (f_p < vide) ? 200 : 368;
...
integer CurMate;
smallint WhMovesRequired[maxply+1], BlMovesRequired[maxply+1],
         CapturesLeft[maxply+1];

boolean MatePossible(void) {
	 smallint whmoves, blmoves, index, time, captures= CapturesLeft[nbply-1];
    piece f_p;
    square t_sq;
    ...
    ** in the mate */
    ...
    if (nbply == 2 ||
        ...
        for (bnp= boardnum; *bnp; bnp++) {
				square f_sq= *bnp;
            if (((f_p= e[f_sq]) != vide) && (f_p != obs)) {	/* V3.64  TLi */
                index= GetIndex(spec[f_sq]);
                if ((t_sq= Mate[index].sq) != initsquare) {
                    if (WhMovesLeft && index == IndxChP) {
                        square _rn= rn;
                        rn= Mate[GetIndex(spec[rn])].sq;
                        time= FroTo(f_p, f_sq, Mate[index].p, t_sq, true);
                        rn= _rn;
                    } else
                        time= FroTo(f_p, f_sq, Mate[index].p, t_sq, false);
                    if (f_p > vide)
                        whmoves += time;
                    else
								blmoves += time;
...
#ifdef NODEF
boolean IntellPatt(couleur camp) {
	 boolean pattflag;
...
/* declarations */
void ImmobilizeByBlBlock(smallint, smallint, smallint, smallint, square,
								 boolean);
...
void NeutralizeMateGuardingPieces(smallint, smallint, smallint, smallint);
void BlackPieceTo(square, smallint, smallint, smallint, smallint);
void WhitePieceTo(square, smallint, smallint, smallint, smallint);

void StaleStoreMate(smallint blmoves, smallint whmoves, smallint blpcallowed,
						  smallint whpcallowed) {
	 smallint i, index, unused= 0;
	 square *bnp, sq, _rb, _rn;
    Flags sp;

    if (blpcallowed < 0 || whpcallowed < 0 || FlagTimeOut)	/* V3.62  NG */
        return;
...
#ifdef DEBUG
    sprintf(GlobalStr, "unused: %d, WhMovesLeft: %d\n", unused, WhMovesLeft);
    StdString(GlobalStr);
#endif
    ...
    IndxChP= nbrchecking == -1 ?			/* V3.50 TLi */
                 -1 : GetIndex(white[nbrchecking].sp);	/* V3.50 TLi */
    _rb= rb;
    _rn= rn;
    ...
    for (bnp= boardnum; *bnp; bnp++)
        if (e[*bnp] != vide) {
            ...
            if (Mate[index].sq != vide &&
                ...
                m= FroTo(e[*bnp], *bnp, Mate[index].p, Mate[index].sq, 0);
                if (e[*bnp] < vide)
                    blm+= m;
                else
                    whm+= m;
...
void DeposeBlPiece(smallint blmoves, smallint whmoves, smallint blpcallowed,
                   smallint whpcallowed) {
    square *bnp, *isbnp= deposebnp;
#ifdef DEBUG
marge++;Tabulate();
sprintf(GlobalStr, "DeposeBlPiece(%d,%d,%d), *deposebnp=%d\n",
...
StdString(GlobalStr);
    ...
    for (bnp= deposebnp; *bnp; bnp++) {
        if (e[*bnp] == vide) {
#ifdef DEBUG
            StdString("deposing piece on ");WriteSquare(*bnp); StdString(" ");
#endif
            deposebnp= bnp;
				ImmobilizeByBlBlock(blmoves, whmoves,
										  blpcallowed, whpcallowed, *bnp, false);
        }
    }
...
#ifdef DEBUG
Tabulate();
sprintf(GlobalStr,"leaving DeposeBlPiece, deposebnp=%d\n", *deposebnp);
StdString(GlobalStr);
marge--;
#endif
} /* DeposeBlPiece */

void PreventCheckAgainstWhK(smallint blmoves, smallint whmoves,
                            smallint blpc, smallint whpc) {
	 square trouble= initsquare;

	 genmove(noir);

/*	 while(empile && (trouble == initsquare)) {	*/
	 while(encore() && (trouble == initsquare)) {	/* V3.65  NG, TLi */
		  if (ca[nbcou] == rb)
			trouble= cd[nbcou];
                  nbcou--;
#ifdef NODEF	/* V3.65  TLi */
		  if (jouecoup() && (ca[nbcou] == rb)) {
				trouble= cd[nbcou];
		  }
		  repcoup();
#endif /* NODEF */
	 ...
	 return;
} /* PreventCheckAgainstWhK */

void StoreMate(smallint blmoves, smallint whmoves, smallint blpc, smallint whpc) {
	 smallint i, index;
	 square *bnp, sq, _rb, _rn;
	 Flags sp;
...
void PinBlPiece(square topin, smallint blmoves, smallint whmoves,
							  smallint blpc, smallint whpc) {
	 square sq= topin;
	 smallint dir, time, i;
	 boolean diagonal;
	 ...
	 while (e[sq+=dir] == vide) {
		  for (i= 1; i < MaxPieceWhite; i++)
				if (!whiteused[i] && (f_p= white[i].p) != Knight) {
					 ...
					 if (f_p == Pawn) {
						  if (diagonal) {
								time= FroTo(f_p, white[i].sq, Bishop, sq, false);
								if (time <= whmoves) {
...
void ImmobilizeByPin(smallint blmoves, smallint whmoves, smallint blpcallowed,
							smallint whpcallowed, square topin) {
	 smallint dir= CheckDirQueen[topin-rn], time, i;
	 boolean diagonal= SquareCol(rn+dir) == SquareCol(rn);
	 square sq;
	 piece f_p;
    ...
    while (e[sq+=dir] == vide) {
        for (i= 1; i < MaxPieceWhite; i++)
            if (!whiteused[i] && ((f_p= white[i].p) != Knight)) {
                ...
                if (f_p == Pawn) {
                    if (diagonal) {
                        if ((time= FroTo(f_p, white[i].sq, Bishop, sq, false))
                                 <= whmoves) {
                            SetPiece(Bishop, sq, white[i].sp);
									 if (patt(noir))
                                ...
                                Immobilize(blmoves, whmoves-time, blpcallowed-1,
                                           whpcallowed);
                        }
                    } else {
                        if ((time= FroTo(f_p, white[i].sq, Rook, sq, false))
                                 <= whmoves) {
                            SetPiece(Rook, sq, white[i].sp);
									 if (patt(noir))
                                StaleStoreMate(blmoves, whmoves-time,
                    ...
                    if ((time= FroTo(f_p, white[i].sq, Queen, sq, false))
                             <= whmoves) {
                        SetPiece(Queen, sq, white[i].sp);
								if (patt(noir))
                            StaleStoreMate(blmoves, whmoves-time, blpcallowed-1,
                                           whpcallowed);
                        else
                            Immobilize(blmoves, whmoves-time, blpcallowed-1,
                                       whpcallowed);
                    ...
                    if ((time= FroTo(f_p, white[i].sq, f_p, sq, false))
                             <= whmoves) {
                        SetPiece(f_p, sq, white[i].sp);
								if (patt(noir))
                            StaleStoreMate(blmoves, whmoves-time, blpcallowed-1,
...
void DeposeWhKing(smallint blmoves, smallint whmoves,
                  smallint blpcallowed, smallint whpcallowed) {
	 piece f_p;

#ifdef DEBUG
marge++;Tabulate();
sprintf(GlobalStr, "entering DeposeWhKing(%d,%d,%d,%d)\n",
    ...
    if (!IllegalCheck(noir) && !IllegalCheck(blanc)) {
        if (echecc(noir)) {
            AvoidCheckInStalemate(blmoves, whmoves,
				...
				if (patt(noir))
					 StaleStoreMate(blmoves, whmoves,
                               blpcallowed, whpcallowed);
            else
                Immobilize(blmoves, whmoves, blpcallowed,
                           whpcallowed);
        }
	 }
    e[rb]= vide;
...
void ImmobilizeByBlBlock(smallint blmoves, smallint whmoves,
								 smallint blpcallowed, smallint whpcallowed,
                         square toblock, boolean morethanonecheck) {
    smallint i, time, pcreq;
	 piece f_p;

#ifdef DEBUG
marge++;Tabulate();
sprintf(GlobalStr, "entering ImmobilizeByBlBlock(%d,%d,%d)\n",
    ...
    for (i= 1; i < MaxPieceBlack; i++)
		  if (!blackused[i]) {
            ...
            if (f_p == -Pawn) {
               ...
               if (moves > 5) /* double step possible */
                   moves= 5;
					if (toblock > 207)   /* square is not on 1st rank -- 1 move
												 * necessary to get there */
                   moves++;
               if (blmoves >= moves) {
                   piece pp= -getprompiece[vide];
                   while (pp != vide) {
                       if (((time= FroTo(f_p, black[i].sq, -pp, toblock, false))
                              <= blmoves) &&
								  (rb == initsquare || !BlIllegalCheck(toblock, pp))) {
            ...
            if ((f_p != -Pawn ) || (toblock > 207)) {
                if (((time= FroTo(f_p, black[i].sq, f_p, toblock, false))
                          <= blmoves)
                    && (pcreq <= blpcallowed)
                    && (rb == initsquare || !BlIllegalCheck(toblock, f_p))) {
                    ...
                    if (morethanonecheck) {
                        AvoidCheckInStalemate(blmoves-time, whmoves,
                            ...
                            Immobilize(blmoves-time, whmoves, blpcallowed-pcreq,
                                       whpcallowed-1);
                        }
                    }
                }
...
void ImmobilizeByWhBlock(smallint blmoves, smallint whmoves,
                         smallint blpcallowed, smallint whpcallowed,
                         square toblock) {
    smallint i, time, pcreq;
	 piece f_p;
...
#ifdef DEBUG
marge++;Tabulate();
sprintf(GlobalStr, "entering ImmobilizeByWhBlock(%d,%d,%d)\n",
		  blmoves, whmoves, toblock);
StdString(GlobalStr);
#endif
...
/* take care of wKing in check/absence !!! */
        if (!whiteused[i]) {
            ...
            if (f_p == Pawn) {
                 ...
                 * promotions.
                 */
                if (whmoves >= ((toblock < 368) ? moves_to_prom[i]+1
																: moves_to_prom[i])) {
                    piece pp= getprompiece[vide];
                    while (pp != vide) {
                        if ((time= FroTo(f_p, white[i].sq, pp, toblock, false))
                                  <= whmoves) {
                            SetPiece(pp, toblock, white[i].sp);
                            if (!IllegalCheck(noir)) {
                                if (echecc(noir)) {
												AvoidCheckInStalemate(blmoves, whmoves-time,
                                                   blpcallowed-1, whpcallowed);
                                } else {
                                    if (patt(noir))
                                        StaleStoreMate(blmoves, whmoves-time,
            ...
            if ((time= FroTo(f_p, white[i].sq, f_p, toblock, false))
							<= whmoves) {
                smallint decpc= i ? 1 : 0;
                SetPiece(f_p, toblock, white[i].sp);
                if (i == 0)
                    rb= toblock;
                if (!IllegalCheck(noir) && (i > 0 || !IllegalCheck(blanc))) {
                    if (echecc(noir)) {
                        AvoidCheckInStalemate(blmoves, whmoves-time,
                        ...
                        if (patt(noir))
                            StaleStoreMate(blmoves, whmoves-time,
									 ...
									 Immobilize(blmoves, whmoves-time, blpcallowed-decpc,
                                       whpcallowed-pcreq);
                    }
                }
            }
...
void Immobilize(smallint blmoves, smallint whmoves, smallint blpcallowed,
                smallint whpcallowed) {
    square trouble= initsquare, block, *bnp, blblock;
    smallint i, blockwhite= 0, blockblack= 0, bpl= 0, wpl= 0, mtba, weight;
    boolean nopinpossible= true, pinnecessary= false;
	 echiquier toblock;
    
#ifdef DEBUG
marge++;Tabulate();sprintf(GlobalStr,"entering Immobilize(%d, %d,%d,%d)\n", blmoves, whmoves, blpcallowed, whpcallowed);StdString(GlobalStr);
#endif
    ...
    for (i= 1; (i < MaxPieceWhite) && nopinpossible; i++)
        nopinpossible= whiteused[i] || white[i].p == Knight
        ...
        if (jouecoup() && !echecc(noir)) {
            ...
            switch (-e[ca[nbcou]]) {
              ...
              case King:
                ...
                break;
              default:
                block= cd[nbcou] + CheckDirQueen[(ca[nbcou]-cd[nbcou])];
            }
            pinnecessary= (ca[nbcou] == block) && (pprise[nbply] != vide);
    ...
    if (!nopinpossible) {
        ImmobilizeByPin(blmoves, whmoves, blpcallowed, whpcallowed, trouble);
    }
    ...
    if (!pinnecessary) {
        ...
        ImmobilizeByBlBlock(blmoves, whmoves, blpcallowed, whpcallowed, blblock,
                            false);

        if ((blblock == block) && (-e[trouble] == Pawn)) {
            ImmobilizeByWhBlock(blmoves, whmoves, blpcallowed, whpcallowed,
                                block);
        }
	 }
...
void AvoidCheckInStalemate(smallint blmoves, smallint whmoves,
									smallint blpcallowed, smallint whpcallowed) {
    smallint dir, checkdirs[8], md= 0, i;

#ifdef DEBUG
marge++;
Tabulate();
sprintf(GlobalStr, "entering AvoidCheckInStaleMate(%d, %d)\n",
                   blmoves, whmoves);
StdString(GlobalStr);
#endif
    if (blpcallowed < 0 || whpcallowed < 0)
...
printf("this message should not appear\n");
    ...
    for (i= 0; i < md; i++) {
        ...
        if ((e[sq+checkdirs[i]] == db) && (SquareCol(sq) != SquareCol(rn)))
            ImmobilizeByBlBlock(blmoves, whmoves, blpcallowed, whpcallowed, sq,
                                md-1);
        if (checkdirs[i] > 0)
            /* actually only white pawns possible */
				ImmobilizeByWhBlock(blmoves, whmoves, blpcallowed, whpcallowed, sq);
#endif
		  square sq= rn;
...
void BlackPieceTo(square sq, smallint blmoves, smallint whmoves,
									  smallint blpc, smallint whpc) {
				...
				if (p != -Pawn || sq > 207) {
					 ...
					 if ((time <= blmoves) &&
                        ...
                        if (diffcol < 0)
                            diffcol= -diffcol;
                        if (diffcol <= blpc)
                            StoreMate(blmoves-time, whmoves, blpc-diffcol, whpc);
                    } else
				...
				if (p == -Pawn) {
					 ...
					 ** promotions. */
					 ...
					 if (time <= blmoves) {
						  ...
						  while (pp != vide) {
								smallint diffcol;
								time= FroTo(p, black[actpbl].sq, -pp, sq, false);
								/* black piece */
								if (pp == -Bishop &&
									 SquareCol(sq) !=
								...
								if ((diffcol <= blpc && time <= blmoves)
									 && (rb == initsquare || !BlIllegalCheck(sq, pp))) {
									 Flags sp= black[actpbl].sp;
									 SetPiece(pp, sq, sp);
									 StoreMate(blmoves-time, whmoves, blpc-diffcol, whpc);
								}
...
void WhitePieceTo(square sq, smallint blmoves, smallint whmoves,
									  smallint blpc, smallint whpc) {
					 ...
					 if (time <= whmoves){
						  ...
						  if (p == Pawn) {
								...
								if (diffcol < 0)
									 diffcol= -diffcol;
								if (diffcol <= whpc)
									 StoreMate(blmoves, whmoves-time, blpc, whpc-diffcol);
						  } else
								StoreMate(blmoves, whmoves-time, blpc, whpc);
					 }
				}
...
void NeutralizeMateGuardingPieces(smallint blmoves, smallint whmoves,
											 smallint blpc, smallint whpc) {
	 square trouble= initsquare, trto;
		  ...
		  if (p == -Pawn) {
				...
				** promotions. */
				...
				if (blmoves >= moves) {
					 piece pp= -getprompiece[vide];
					 while (pp != vide) {
						  if ((time= FroTo(p, black[i].sq, -pp, sq, false)) < mintime)
								mintime= time;
						  pp= -getprompiece[-pp];
					 }
				}
...
void GenerateBlocking(smallint whmoves, smallint nbrfl, square *toblock,
							 smallint *mintime, smallint blpcallowed,
							 smallint whpcallowed, smallint timetowaste) {
	 square sq, *bnp;
	 smallint i, actpbl, wasted;
	 ...
	 if (nbrfl == 0) {
		  ...
		  if (stipulation == stip_stale || echecc(noir)) {	/* V3.50  TLi */
				...
				if (stipulation == stip_stale) {
					 if (echecc(noir)) {
						  AvoidCheckInStalemate(timetowaste, whmoves, blpcallowed,
								whpcallowed);
					 } else {
						  if (patt(noir))
								StaleStoreMate(timetowaste, whmoves, blpcallowed,
									 whpcallowed);
						  else
								Immobilize(timetowaste, whmoves, blpcallowed,
									 whpcallowed);
					 }
				} else
  				    StoreMate(timetowaste, whmoves, blpcallowed, whpcallowed);
		  }
	 } else {
/*        square sq;	*/	/* Already declared above ...  V3.64  NG */
                    ...
                    if ((wasted <= timetowaste) &&
                        ...
                        if (p == -Pawn) {
                            ...
                            if (diffcol <= blpcallowed)
                                GenerateBlocking(whmoves, nbrfl, toblock,
                                    mintime, blpcallowed-diffcol, whpcallowed,
                                    timetowaste-wasted);
                        } else
                            GenerateBlocking(whmoves, nbrfl, toblock, mintime,
                                blpcallowed, whpcallowed, timetowaste-wasted);
                    }
                }
                /* pawn promotions */
                if (p == -Pawn) {
                    ...
                    ** promotions. */
                    ...
                    while (pp != vide) {
                        smallint diffcol;
                        wasted= FroTo(p, black[actpbl].sq, -pp, sq, false)-
                                mintime[nbrfl];
                        /* black piece */
                        if (pp == -Bishop &&
                            SquareCol(sq) !=
                            ...
                            diffcol= 0;
                        if ((diffcol <= blpcallowed && wasted <= timetowaste)
                            && (rb == initsquare || !BlIllegalCheck(sq, pp))) {
                            Flags sp= black[actpbl].sp;
...
void GenerateGuarding(smallint actpwh, smallint whmoves,
                      smallint blmoves, smallint whcaptures) {
    square *bnp, toblock[8];
    smallint i, flights;
    boolean unblockable= false;

#ifdef DEBUG
sprintf(GlobalStr, "GenerateGuarding(%d, %d, %d, %d)\n",
...
#ifdef DEBUG
        WritePosition();
        sprintf(GlobalStr, "flights: %d, ", flights); StdString(GlobalStr);
        sprintf(GlobalStr, "whcaptures: %d\n", whcaptures); StdString(GlobalStr);
#endif
        if (whcaptures+flights > MaxPieceBlack-1)
            return;

        mtba= 0;
        for (i= 0; i < flights && mtba <= blmoves; i++)   /* V3.62  TLi */
...
#ifdef DEBUG
sprintf(GlobalStr, "GenerateBlocking with timetowaste=%d\n", blmoves-mtba);
StdString(GlobalStr);
#endif
        ...
        if (set_king) {			/* V3.64  TLi */
            ...
            whiteused[0]= true;
            GenerateBlocking(whmoves, flights, toblock,
               mintime, blpcallowed, MaxPieceBlack-1-whcaptures, blmoves-mtba);
            whiteused[0]= false;
            e[rb]= vide;
            spec[rb]= EmptySpec;
            rb= initsquare;
        ...
        for (bnp= boardnum; *bnp; bnp++) {
            if (e[*bnp] == vide) {
                /* set piece */
                smallint time= FroTo(p, sq, p, *bnp, false);
                if (actpwh == 0) {
                ...
                if (p == Pawn) {
                    /* A rough check whether it is worth thinking about
                    ** promotions. */
                    if (whmoves >= ((*bnp < 368) ? moves_to_prom[actpwh]+1
            ...
            if (e[sq= boardnum[i]] == vide) {
                ...
                if (time <= whmoves && guards(rn, p, sq)) {
                    ...
                    if (p == Pawn) {
                        ...
                        GenerateGuarding(0, whmoves-time,
                                         blmoves, abs(diffcol));
                    } else
                ...
                if (p == Pawn) {
                    /* A rough check whether it is worth thinking about
                    ** promotions. */
                    if (whmoves >= ((sq < 368) ? moves_to_prom[j]+1
                                               : moves_to_prom[j])) {
                        piece pp= getprompiece[vide];
                        while (pp != vide) {
...
void GenerateBlackKing(smallint whmoves, smallint blmoves) {
    ...
    for (i= 0; i < 64; i++) {
        if (e[sq= boardnum[i]] != obs) {		/* V3.64  TLi */
            /* set piece */
        ...
        if ((rb != *bnp) && (e[*bnp] > obs)) {		/* V3.64  TLi */
            ...
            if (e[*bnp] == Pawn) {
                ...
                if (whmoves < 7 && sq < 248
                    ...
                    && (e[sq+24] == -Pawn
                        || (e[sq+23] <= King && e[sq+25] <= King
									 && (ep[1] != sq+23) && (ep[1] != sq+25)
									 && e[sq+48] == -Pawn)))
                        moves++;
