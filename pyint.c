...
/* declarations */
static void stalemate_block_square_black(unsigned int nr_remaining_black_moves,
...
static void stalemate_place_an_unused_black_piece(unsigned int nr_remaining_black_moves,
                                                  ...
                                                  unsigned int max_nr_allowed_captures_of_white_pieces,
                                                  stip_length_type n);
static void stalemate_immobilise(unsigned int nr_remaining_black_moves,
                                 unsigned int nr_remaining_white_moves,
                                 unsigned int max_nr_allowed_captures_of_black_pieces,
                                 unsigned int max_nr_allowed_captures_of_white_pieces,
                                 stip_length_type n);
...
static void stalemate_store_target_position(unsigned int nr_remaining_black_moves,
                                            unsigned int nr_remaining_white_moves,
                                            unsigned int max_nr_allowed_captures_of_black_pieces,
                                            unsigned int max_nr_allowed_captures_of_white_pieces,
                                            stip_length_type n)
    ...
    for (bnp= boardnum; *bnp; bnp++)
      if (e[*bnp] != vide)
      {
        ...
        if (target_position[id].sq != vide
            && (*bnp != target_position[id].sq || e[*bnp] != target_position[id].p))
        {
          WritePiece(e[*bnp]); WriteSquare(*bnp);
          StdString("-->");
          if (e[*bnp] != target_position[id].p) {
            WritePiece(target_position[id].p);
          }
          WriteSquare(target_position[id].sq);
          m= count_nr_of_moves_from_to_no_check(e[*bnp],*bnp,target_position[id].p,target_position[id].sq,0);
          if (e[*bnp] < vide)
            blm+= m;
          else
            whm+= m;
                                                  ...
                                                  stip_length_type n)
{
  ...
  sprintf(GlobalStr,
          ...
          nr_remaining_black_moves,nr_remaining_white_moves,max_nr_allowed_captures_of_black_pieces,*deposebnp);
  StdString(GlobalStr);
#endif
...
#endif
      deposebnp= bnp;
      stalemate_block_square_black(nr_remaining_black_moves,
                                   nr_remaining_white_moves,
                                   max_nr_allowed_captures_of_black_pieces,
                                   max_nr_allowed_captures_of_white_pieces,
                                   *bnp,
...
static void mate_store_target_position(unsigned int nr_remaining_black_moves,
                                       unsigned int nr_remaining_white_moves,
                                       unsigned int max_nr_allowed_captures_of_black_pieces,
                                       unsigned int max_nr_allowed_captures_of_white_pieces,
                                       stip_length_type n)
...

  {
    ...
    if (time<=nr_remaining_white_moves)
    {
      ...
      if (slice_has_solution(slices[current_start_slice].u.fork.fork)
          ==has_solution)
        stalemate_store_target_position(nr_remaining_black_moves,
                                        nr_remaining_white_moves-time,
                                        max_nr_allowed_captures_of_black_pieces-1,
                                        max_nr_allowed_captures_of_white_pieces,
                                        n);
      else
        stalemate_immobilise(nr_remaining_black_moves,
                             nr_remaining_white_moves-time,
                             max_nr_allowed_captures_of_black_pieces-1,
                             max_nr_allowed_captures_of_white_pieces,
                             n);
...
static void stalemate_continue_after_blocking_check_black(unsigned int nr_remaining_black_moves,
                                                          unsigned int nr_remaining_white_moves,
                                                          unsigned int max_nr_allowed_captures_of_black_pieces,
                                                          unsigned int max_nr_allowed_captures_of_white_pieces,
                                                          unsigned int nr_of_checks,
                                                          stip_length_type n)
{
  TraceFunctionEntry(__func__);
  TraceFunctionParam("%u",nr_remaining_black_moves);
  ...
  TraceFunctionParam("%u",max_nr_allowed_captures_of_black_pieces);
  TraceFunctionParam("%u",max_nr_allowed_captures_of_white_pieces);
  TraceFunctionParam("%u",nr_of_checks);
  TraceFunctionParam("%u",n);
  TraceFunctionParamListEnd();
...
static void stalemate_block_check_with_promoted_black_pawn(unsigned int nr_remaining_black_moves,
                                                           unsigned int nr_remaining_white_moves,
                                                           unsigned int max_nr_allowed_captures_of_black_pieces,
                                                           unsigned int max_nr_allowed_captures_of_white_pieces,
                                                           square to_be_blocked,
  ...
  if (nr_remaining_black_moves>=time)
  {
    ...
    for (pp = -getprompiece[vide]; pp!=vide; pp = -getprompiece[-pp])
    {
          ...
          && !uninterceptably_attacks_white_king(to_be_blocked,pp))
      {
        ...
        stalemate_continue_after_blocking_check_black(nr_remaining_black_moves-time,
                                                      nr_remaining_white_moves,
                                                      max_nr_allowed_captures_of_black_pieces,
                                                      max_nr_allowed_captures_of_white_pieces,
                                                      nr_of_checks,
...
static void stalemate_block_check_with_unpromoted_black_pawn(unsigned int nr_remaining_black_moves,
                                                             unsigned int nr_remaining_white_moves,
                                                             unsigned int max_nr_allowed_captures_of_black_pieces,
                                                             unsigned int max_nr_allowed_captures_of_white_pieces,
                                                             square to_be_blocked,
...
static void stalemate_block_check_with_black_officer(unsigned int nr_remaining_black_moves,
                                                     unsigned int nr_remaining_white_moves,
                                                     unsigned int max_nr_allowed_captures_of_black_pieces,
                                                     unsigned int max_nr_allowed_captures_of_white_pieces,
                                                     square to_be_blocked,
...
static void stalemate_block_check_black(unsigned int nr_remaining_black_moves,
                                        unsigned int nr_remaining_white_moves,
                                        unsigned int max_nr_allowed_captures_of_black_pieces,
                                        unsigned int max_nr_allowed_captures_of_white_pieces,
                                        square to_be_blocked,
  ...
  if (max_nr_allowed_captures_of_white_pieces>=1)
  {
    ...
    for (i = 1; i<MaxPiece[Black]; ++i)
      if (!black[i].used)
      {
        ...
        if (blocker_type==pn)
        {
          stalemate_block_check_with_promoted_black_pawn(nr_remaining_black_moves,
                                                         nr_remaining_white_moves,
                                                         max_nr_allowed_captures_of_black_pieces,
                                                         max_nr_allowed_captures_of_white_pieces,
                                                         to_be_blocked,
                                                         ...
                                                         n);
          if (to_be_blocked>=square_a2)
            stalemate_block_check_with_unpromoted_black_pawn(nr_remaining_black_moves,
                                                             nr_remaining_white_moves,
                                                             max_nr_allowed_captures_of_black_pieces,
                                                             max_nr_allowed_captures_of_white_pieces,
                                                             to_be_blocked,
          ...
          stalemate_block_check_with_black_officer(nr_remaining_black_moves,
                                                   nr_remaining_white_moves,
                                                   max_nr_allowed_captures_of_black_pieces,
                                                   max_nr_allowed_captures_of_white_pieces,
                                                   to_be_blocked,
...
static void stalemate_continue_after_blocking_square_black(unsigned int nr_remaining_black_moves,
                                                           unsigned int nr_remaining_white_moves,
                                                           unsigned int max_nr_allowed_captures_of_black_pieces,
                                                           unsigned int max_nr_allowed_captures_of_white_pieces,
                                                           stip_length_type n)
...
static void stalemate_block_square_with_promoted_black_pawn(unsigned int nr_remaining_black_moves,
                                                            unsigned int nr_remaining_white_moves,
                                                            unsigned int max_nr_allowed_captures_of_black_pieces,
                                                            unsigned int max_nr_allowed_captures_of_white_pieces,
                                                            square to_be_blocked,
                                                            ...
                                                            stip_length_type n)
{
  ...
  if (nr_remaining_black_moves>=time)
  {
    ...
    for (pp = -getprompiece[vide]; pp!=vide; pp = -getprompiece[-pp])
    {
      ...
      {
        SetPiece(pp,to_be_blocked,blocker_flags);
        stalemate_continue_after_blocking_square_black(nr_remaining_black_moves-time,
                                                       nr_remaining_white_moves,
                                                       max_nr_allowed_captures_of_black_pieces,
                                                       max_nr_allowed_captures_of_white_pieces,
                                                       n);
...
static void stalemate_block_square_with_unpromoted_black_pawn(unsigned int nr_remaining_black_moves,
                                                              unsigned int nr_remaining_white_moves,
                                                              unsigned int max_nr_allowed_captures_of_black_pieces,
                                                              unsigned int max_nr_allowed_captures_of_white_pieces,
                                                              square to_be_blocked,
...
static void stalemate_block_square_with_black_officer(unsigned int nr_remaining_black_moves,
                                                      unsigned int nr_remaining_white_moves,
                                                      unsigned int max_nr_allowed_captures_of_black_pieces,
                                                      unsigned int max_nr_allowed_captures_of_white_pieces,
                                                      square to_be_blocked,
  ...
  if (max_nr_allowed_captures_of_white_pieces>=1)
  {
    ...
    for (i = 1; i<MaxPiece[Black]; ++i)
      if (!black[i].used)
      {
        ...
        if (blocker_type==pn)
        {
          stalemate_block_square_with_promoted_black_pawn(nr_remaining_black_moves,
                                                          nr_remaining_white_moves,
                                                          max_nr_allowed_captures_of_black_pieces,
                                                          max_nr_allowed_captures_of_white_pieces,
                                                          to_be_blocked,
                                                          ...
                                                          n);
          if (to_be_blocked>=square_a2)
            stalemate_block_square_with_unpromoted_black_pawn(nr_remaining_black_moves,
                                                              nr_remaining_white_moves,
                                                              max_nr_allowed_captures_of_black_pieces,
                                                              max_nr_allowed_captures_of_white_pieces,
                                                              to_be_blocked,
          ...
          stalemate_block_square_with_black_officer(nr_remaining_black_moves,
                                                    nr_remaining_white_moves,
                                                    max_nr_allowed_captures_of_black_pieces,
                                                    max_nr_allowed_captures_of_white_pieces,
                                                    to_be_blocked,
...
static void stalemate_continue_after_blocking_check_white(unsigned int nr_remaining_black_moves,
                                                          unsigned int nr_remaining_white_moves,
                                                          unsigned int max_nr_allowed_captures_of_black_pieces,
                                                          unsigned int max_nr_allowed_captures_of_white_pieces,
                                                          unsigned int nr_of_checks,
...
static void stalemate_block_check_with_unpromoted_white_pawn(unsigned int blocker_index,
                                                             unsigned int nr_remaining_black_moves,
                                                             unsigned int nr_remaining_white_moves,
                                                             unsigned int max_nr_allowed_captures_of_black_pieces,
                                                             unsigned int max_nr_allowed_captures_of_white_pieces,
                                                             ...
                                                             stip_length_type n)
{
...

  {
    ...
    unsigned int const time = count_nr_of_moves_from_to_no_check(pb,
                                                                 ...
                                                                 to_be_blocked);
    if (time<=nr_remaining_white_moves
        && !guards(king_square[Black],pb,to_be_blocked))
    {
      SetPiece(pb,to_be_blocked,white[blocker_index].sp);

      if (max_nr_allowed_captures_of_white_pieces>=nr_captures_required)
        stalemate_continue_after_blocking_check_white(nr_remaining_black_moves,
                                                      nr_remaining_white_moves-time,
                                                      max_nr_allowed_captures_of_black_pieces,
...
static void stalemate_block_check_with_promoted_white_pawn(unsigned int blocker_index,
                                                           unsigned int nr_remaining_black_moves,
                                                           unsigned int nr_remaining_white_moves,
                                                           unsigned int max_nr_allowed_captures_of_black_pieces,
                                                           unsigned int max_nr_allowed_captures_of_white_pieces,
  ...
  if (nr_remaining_white_moves>=nr_moves_required)
  {
    ...
    for (pp = getprompiece[vide]; pp!=vide; pp = getprompiece[pp])
    {
      unsigned int const time = count_nr_of_moves_from_to_no_check(pb,
                                                                   ...
                                                                   to_be_blocked);
      if (time<=nr_remaining_white_moves
          && !guards(king_square[Black],pp,to_be_blocked))
      {
...
static void stalemate_block_check_with_white_king(unsigned int nr_remaining_black_moves,
                                                  unsigned int nr_remaining_white_moves,
                                                  unsigned int max_nr_allowed_captures_of_black_pieces,
                                                  unsigned int max_nr_allowed_captures_of_white_pieces,
                                                  square to_be_blocked,
...
static void stalemate_block_check_with_white_officer(piece blocker_type,
...
static void stalemate_block_check_white(unsigned int nr_remaining_black_moves,
                                        unsigned int nr_remaining_white_moves,
                                        unsigned int max_nr_allowed_captures_of_black_pieces,
                                        unsigned int max_nr_allowed_captures_of_white_pieces,
                                        square to_be_blocked,
                                        ...
                                        stip_length_type n)
{
  ...
  {
    white[index_of_king].used = true;
    stalemate_block_check_with_white_king(nr_remaining_black_moves,
                                          nr_remaining_white_moves,
                                          max_nr_allowed_captures_of_black_pieces,
                                          max_nr_allowed_captures_of_white_pieces,
                                          to_be_blocked,
  ...
  if (max_nr_allowed_captures_of_black_pieces>=1)
  {
    ...
    for (blocker_index = 1; blocker_index<MaxPiece[White]; ++blocker_index)
      if (!white[blocker_index].used)
      {
        ...
        if (blocker_type==pb)
        {
          stalemate_block_check_with_promoted_white_pawn(blocker_index,
                                                         nr_remaining_black_moves,
                                                         nr_remaining_white_moves,
                                                         max_nr_allowed_captures_of_black_pieces,
                                                         max_nr_allowed_captures_of_white_pieces,
                                                         ...
                                                         nr_of_checks,
                                                         n);
          stalemate_block_check_with_unpromoted_white_pawn(blocker_index,
                                                           nr_remaining_black_moves,
                                                           nr_remaining_white_moves,
                                                           max_nr_allowed_captures_of_black_pieces,
                                                           max_nr_allowed_captures_of_white_pieces,
          ...
          stalemate_block_check_with_white_officer(blocker_type,blocker_index,
                                                   nr_remaining_black_moves,
                                                   nr_remaining_white_moves,
                                                   max_nr_allowed_captures_of_black_pieces,
                                                   max_nr_allowed_captures_of_white_pieces,
...
static void stalemate_continue_after_blocking_square_white(unsigned int nr_remaining_black_moves,
                                                           unsigned int nr_remaining_white_moves,
                                                           unsigned int max_nr_allowed_captures_of_black_pieces,
                                                           unsigned int max_nr_allowed_captures_of_white_pieces,
                                                           stip_length_type n)
...
static void stalemate_block_square_with_white_king(unsigned int nr_remaining_black_moves,
                                                   unsigned int nr_remaining_white_moves,
                                                   unsigned int max_nr_allowed_captures_of_black_pieces,
                                                   unsigned int max_nr_allowed_captures_of_white_pieces,
                                                   square to_be_blocked,
...
static void stalemate_block_square_white(unsigned int nr_remaining_black_moves,
                                         unsigned int nr_remaining_white_moves,
                                         unsigned int max_nr_allowed_captures_of_black_pieces,
                                         unsigned int max_nr_allowed_captures_of_white_pieces,
                                         square to_be_blocked,
      ...
      switch (most_expensive_square_to_be_blocked_by_black)
      {
                 ...
                 ==white_block_sufficient_on_square);
        {
          ...
          stalemate_block_square_black(nr_remaining_black_moves,
                                       ...
                                       immobilisation_state.last_found_trouble_square,
                                       n);
          stalemate_block_square_white(nr_remaining_black_moves,
                                       nr_remaining_white_moves,
                                       max_nr_allowed_captures_of_black_pieces,
                                       max_nr_allowed_captures_of_white_pieces,
                                  ...
                                  stip_length_type n)
{
  ...
  for (i = 0; i<nr_of_checks; i++)
  {
    square to_be_blocked;
    for (to_be_blocked = king_square[side]+checkdirs[i];
         e[to_be_blocked]==vide;
         to_be_blocked += checkdirs[i])
    {
      stalemate_block_check_black(nr_remaining_black_moves,
                                  nr_remaining_white_moves,
                                  max_nr_allowed_captures_of_black_pieces,
                                  max_nr_allowed_captures_of_white_pieces,
                                  to_be_blocked,
                                  nr_of_checks,
                                  n);
      stalemate_block_check_white(nr_remaining_black_moves,
                                  nr_remaining_white_moves,
                                  max_nr_allowed_captures_of_black_pieces,
                                  max_nr_allowed_captures_of_white_pieces,
                                  to_be_blocked,
                              ...
                              stip_length_type n)
{
  ...
  if (goal_to_be_reached==goal_stale)
  {
    if (echecc(nbply,Black))
      ...
      stalemate_store_target_position(timetowaste,
                                      nr_remaining_white_moves,
                                      max_nr_allowed_captures_of_black_pieces,
                                      max_nr_allowed_captures_of_white_pieces,
                                      n);
    else
      stalemate_immobilise(timetowaste,
                           nr_remaining_white_moves,
                           max_nr_allowed_captures_of_black_pieces,
                           max_nr_allowed_captures_of_white_pieces,
                           n);
    ...
    mate_store_target_position(timetowaste,
                               nr_remaining_white_moves,
                               max_nr_allowed_captures_of_black_pieces,
                               max_nr_allowed_captures_of_white_pieces,
                               n);
...
static void mate_generate_checking_move_by_one_promoted_pawn(unsigned int nr_remaining_white_moves,
                                                             unsigned int nr_remaining_black_moves,
                                                             stip_length_type n)
{
      ...
      if (nr_remaining_white_moves>=min_nr_moves_by_p)
      {
        ...
        for (pp = getprompiece[vide]; pp!=vide; pp = getprompiece[pp])
        {
          unsigned int const time= count_nr_of_moves_from_to_no_check(pb,
                                                                      ...
                                                                      check_from);
          if (time<=nr_remaining_white_moves
              && guards(king_square[Black],pp,check_from))
          {
...
static void mate_generate_checking_move_by_one_unpromoted_pawn(unsigned int nr_remaining_white_moves,
                                                               unsigned int nr_remaining_black_moves,
                                                               stip_length_type n)
{
  Flags const checker_flags = white[index_of_piece_delivering_check].sp;
  ...
  for (i = 0; i<nr_squares_on_board; ++i)
  {
    ...
    if (e[check_from]==vide)
    {
      ...
      unsigned int const time = count_nr_of_moves_from_to_checking(pb,
                                                                   ...
                                                                   check_from);
      if (time<=nr_remaining_white_moves
          && guards(king_square[Black],pb,check_from))
      {
        unsigned int const min_nr_white_captures = abs(pawn_origin%onerow - check_from%onerow);
  ...
  for (i = 0; i<nr_squares_on_board; ++i)
  {
    ...
    if (e[check_from]==vide)
    {
      ...
      unsigned int const time = count_nr_of_moves_from_to_checking(checker_type,
                                                                   ...
                                                                   check_from);
      if (time<=nr_remaining_white_moves
          && guards(king_square[Black],checker_type,check_from))
      {
        unsigned int const min_nr_white_captures = 0;
                                        ...
                                        stip_length_type n)
{
       ...
       ++index_of_piece_delivering_check)
  {
    ...
    if (checker_type==pb)
    {
      mate_generate_checking_move_by_one_unpromoted_pawn(nr_remaining_white_moves,
                                                         nr_remaining_black_moves,
                                                         n);
      mate_generate_checking_move_by_one_promoted_pawn(nr_remaining_white_moves,
                                                       nr_remaining_black_moves,
                                                       n);
    }
    else
      mate_generate_checking_move_by_one_officer(checker_type,
                                                 nr_remaining_white_moves,
                                                 nr_remaining_black_moves,
                                                 n);
