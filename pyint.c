...
boolean IllegalCheck(couleur camp) {
  ...
  if (camp == noir) {
	...
	return checks > (phases[current_phase].goal == stip_stale ? 0 : 1);
  }
  else {
...
boolean MatePossible(void) {
  ...
  */
  ...
  if (phases[current_phase].goal == stip_stale) {
  ...
  int	timetowaste)
{
  ...
  if (nbrfl == 0) {
	...
	if (phases[current_phase].goal == stip_stale || echecc(noir)) {
#if defined(DEBUG)
  ...
  int	whcaptures)
{
  ...
  if (actpwh == MaxPieceWhite) {
	...
	/* check for check */
	if (phases[current_phase].goal == stip_mate && !echecc(noir)) {
      return;
	}
...
void GenerateBlackKing(int whmoves, int blmoves) {
  ...
  for (i= 0; i < nr_squares_on_board; i++) {
	...
	if (time <= blmoves) {
      ...
      StdString("\n");
#endif
      if (phases[current_phase].goal == stip_mate) {
