...
void andernach_assume_side(Side side)
{
  ...
  CLRFLAG(spec[sq_arrival],Black);
  CLRFLAG(spec[sq_arrival],White);
  CLRFLAG(spec[sq_arrival],Neutral);
  SETFLAG(spec[sq_arrival],side);
  replace_arriving_piece(side==White ? abs(e[sq_arrival]) : -abs(e[sq_arrival]));
}
 ...
 *            n+2 no solution found
 */
stip_length_type andernach_castling_rights_restorer_attack(slice_index si,
                                                           stip_length_type n)
{
  stip_length_type result;
 ...
 *         n+2 refuted - >acceptable number of refutations found
 */
stip_length_type andernach_castling_rights_restorer_defend(slice_index si,
                                                           stip_length_type n)
{
  stip_length_type result;
...
void stip_insert_andernach(slice_index si)
{
  ...
  if (castling_supported)
    stip_instrument_moves_no_replay(si,STAndernachCastlingRightsRestorer);
